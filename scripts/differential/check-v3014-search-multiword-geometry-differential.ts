...
import {
  canonicalSearchMultiwordGeometryResult,
  type Json,
} from './v3014-search-multiword-geometry-projection.ts';
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 multiword geometry tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 multiword geometry tree mismatch');
}
for (const [name, path, expected] of [
  ...
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`multiword geometry ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`multiword geometry entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('multiword geometry bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust multiword geometry timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-search-multiword-geometry-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalSearchMultiwordGeometryResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-search-multiword-geometry-relocated-'));
try {
  copyFileSync(fixturePath, join(relocatedDir, 'v3014-behavior-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`multiword geometry comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const start = oracle.expectations['multiword-start-item-union'] as Record<string, Json>;
const mid = oracle.expectations['multiword-mid-line-union'] as Record<string, Json>;
const ci = oracle.expectations['multiword-case-insensitive-union'] as Record<string, Json>;
const startMatch = (((start.results as Json[])[0] as Record<string, Json>).matches as Json[])[0] as Record<
  string,
  Json
>;
const midMatch = (((mid.results as Json[])[0] as Record<string, Json>).matches as Json[])[0] as Record<
  string,
  Json
>;
const ciMatch = (((ci.results as Json[])[0] as Record<string, Json>).matches as Json[])[0] as Record<
  string,
  Json
>;
if (startMatch.bounding_box_level !== 'char_estimated') {
  throw new Error('start multiword must use char_estimated geometry');
}
if (midMatch.bounding_box_level !== 'char_estimated') {
  throw new Error('mid multiword must use char_estimated geometry');
}
if (ciMatch.text !== 'Needle at') {
  throw new Error('case-insensitive multiword must match Needle at');
}
const startBox = startMatch.bounding_box as Record<string, number>;
const midBox = midMatch.bounding_box as Record<string, number>;
if (!(startBox.right > startBox.left) || !(midBox.right > midBox.left)) {
  throw new Error('multiword unions must have positive width');
}
if (startBox.right <= 103.53) {
  throw new Error('start multiword union must extend past single-word Needle width');
}
...
const report = {
  ...
  providerProof: {
    multiwordStartItemUnion: true,
    multiwordMidLineUnion: true,
    multiwordCaseInsensitiveUnion: true,
    charEstimatedLevel: true,
  },
...
console.error(
  `v3.0.14 multiword geometry differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
