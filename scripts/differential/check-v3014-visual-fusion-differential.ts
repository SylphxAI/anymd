...
import { canonicalVisualFusionResult, type Json } from './v3014-visual-fusion-projection.ts';
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 visual-fusion tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 visual-fusion tree mismatch');
}
...
for (const [name, path, expected] of bindings) {
  if (sha256(readFileSync(path)) !== expected) throw new Error(`visual-fusion ${name} digest drift`);
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`visual-fusion entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('visual-fusion bun.lock digest drift');
}
...
const invoke = async (
  ...
  const markerDir = mkdtempSync(join(tmpdir(), 'pdf-reader-visual-fusion-rust-'));
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-visual-fusion-rust', version: '1' },
    });
    child.stdin.write(
    ...
    const invocations = readFileSync(markerPath, 'utf8')
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    return { actual: canonicalVisualFusionResult(response, invocations), invocations };
  } finally {
    child.kill('SIGTERM');
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-visual-fusion-relocated-'));
try {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`visual-fusion comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const zeroCall = oracle.expectations['ready-no-candidate-zero-call'] as Record<string, Json>;
const failureCase = oracle.expectations[
  'second-provider-failure-discards-partial'
...
if (
  Array.isArray(failureData.visual_enrichments) &&
  failureData.visual_enrichments.length !== 0
) {
  throw new Error('provider failure must discard partial enrichments');
}
if (
  !Array.isArray(failureData.visual_enrichment_candidates) ||
  failureData.visual_enrichment_candidates.length !== 2
) {
  throw new Error('provider failure must retain candidates');
}
...
const report = {
  schemaVersion: 1,
  profile: 'pdf_reader_v3014_visual_fusion_result',
  candidateSha,
  ...
  capabilityStatus: { includeVisualEnrichments: 'PARTIAL' },
...
console.error(
  `v3.0.14 visual-fusion differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
