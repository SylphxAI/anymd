...
import { canonicalFormRadioMalformedApResidualResult, type Json } from './v3014-form-radio-malformed-ap-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-malformed-ap-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-malformed-ap-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-malformed-ap-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-malformed-ap-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-malformed-ap residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-malformed-ap residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['radioApStreamFixture', radioApStreamFixture, baseline.fixtureSha256['v3014-form-radio-ap-stream-keeps-v-v1.pdf']!],
  ['radioApnStreamFixture', radioApnStreamFixture, baseline.fixtureSha256['v3014-form-radio-apn-stream-keeps-v-v1.pdf']!],
  ['radioApNamedFixture', radioApNamedFixture, baseline.fixtureSha256['v3014-form-radio-ap-named-keeps-v-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-malformed-ap residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-malformed-ap residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-malformed-ap residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-malformed-ap residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-malformed-ap-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioMalformedApResidualResult(
...
try {
        copyFileSync(radioApStreamFixture, join(relocatedDir, 'v3014-form-radio-ap-stream-keeps-v-v1.pdf'));
  copyFileSync(radioApnStreamFixture, join(relocatedDir, 'v3014-form-radio-apn-stream-keeps-v-v1.pdf'));
  copyFileSync(radioApNamedFixture, join(relocatedDir, 'v3014-form-radio-ap-named-keeps-v-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-malformed-ap residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const radioApStream = fieldsOf('form-radio-ap-stream-keeps-v')[0]!;
const radioApnStream = fieldsOf('form-radio-apn-stream-keeps-v')[0]!;
const radioApNamed = fieldsOf('form-radio-ap-named-keeps-v')[0]!;
if (radioApStream.type !== 'radiobutton' || radioApStream.value !== 'Gold' || radioApStream.default_value !== null) {
  throw new Error('radio AP stream must keep V=/Gold and null default');
}
if (radioApnStream.type !== 'radiobutton' || radioApnStream.value !== 'Gold' || radioApnStream.default_value !== null) {
  throw new Error('radio AP/N stream must keep V=/Gold and null default');
}
if (radioApNamed.type !== 'radiobutton' || radioApNamed.value !== 'Gold' || radioApNamed.default_value !== 'Off') {
  throw new Error('radio named AP/N must keep V=/Gold (no AS override) and default Off');
}
...
const report = {
  ...
  providerProof: {
    radioApStreamKeepsV: true,
    radioApnStreamKeepsV: true,
    radioApNamedKeepsV: true,
  },
...
console.error(
  `v3.0.14 form radio-malformed-ap residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
