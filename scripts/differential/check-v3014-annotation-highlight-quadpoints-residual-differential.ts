...
import {
  canonicalAnnotationHighlightQuadpointsResidualResult,
  type Json,
} from './v3014-annotation-highlight-quadpoints-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-highlight-quadpoints-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-highlight-quadpoints-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-highlight-quadpoints-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-highlight-quadpoints-residual-projection.ts');
const lineApAsMissingFixture = join(fixtureDir, 'v3014-annotation-highlight-quad-noap-v1.pdf');
const lineApAsOnFixture = join(fixtureDir, 'v3014-annotation-highlight-ap-noext-v1.pdf');
const lineApAsInvalidFixture = join(fixtureDir, 'v3014-annotation-highlight-ap-ext-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP highlight quadpoints residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP highlight quadpoints residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    ...
    lineApAsMissingFixture,
    baseline.fixtureSha256['v3014-annotation-highlight-quad-noap-v1.pdf']!,
  ],
  [
    'lineApAsOnFixture',
    lineApAsOnFixture,
    baseline.fixtureSha256['v3014-annotation-highlight-ap-noext-v1.pdf']!,
  ],
  [
    'lineApAsInvalidFixture',
    lineApAsInvalidFixture,
    baseline.fixtureSha256['v3014-annotation-highlight-ap-ext-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP highlight quadpoints residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP highlight quadpoints residual entrypoint digest drift: ${path}`);
  }
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP highlight quadpoints residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-highlight-quadpoints-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationHighlightQuadpointsResidualResult(
...
try {
  copyFileSync(lineApAsMissingFixture, join(relocatedDir, 'v3014-annotation-highlight-quad-noap-v1.pdf'));
  copyFileSync(lineApAsOnFixture, join(relocatedDir, 'v3014-annotation-highlight-ap-noext-v1.pdf'));
  copyFileSync(lineApAsInvalidFixture, join(relocatedDir, 'v3014-annotation-highlight-ap-ext-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP highlight quadpoints residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const noAp = ((
  dataOf('highlight-quad-noap-uses-quadpoints').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const apNoExt = ((
  dataOf('highlight-ap-noext-uses-quadpoints').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const apExt = ((
  dataOf('highlight-ap-ext-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const noApBox = (noAp[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const apNoExtBox = (apNoExt[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const apExtBox = (apExt[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  noApBox.left !== 10 ||
  noApBox.bottom !== 10 ||
  noApBox.right !== 100 ||
  ...
  throw new Error('Highlight without appearance must use QuadPoints bbox');
}
if (
  apNoExtBox.left !== 10 ||
  apNoExtBox.bottom !== 10 ||
  apNoExtBox.right !== 100 ||
  apNoExtBox.top !== 80
) {
  throw new Error('Highlight appearance without ExtGState must use QuadPoints bbox');
}
if (
  apExtBox.left !== 200 ||
  apExtBox.bottom !== 200 ||
  ...
  throw new Error('Highlight appearance with ExtGState must keep raw Rect');
}
if ((noAp[0] as Record<string, Json>).subtype !== 'Highlight') {
  throw new Error('highlight residual subtype must remain Highlight');
}
if ((apNoExt[0] as Record<string, Json>).subtype !== 'Highlight') {
  throw new Error('highlight residual subtype must remain Highlight');
}
if ((apExt[0] as Record<string, Json>).subtype !== 'Highlight') {
  throw new Error('highlight residual subtype must remain Highlight');
}
...
const report = {
  ...
  providerProof: {
    highlightQuadNoApUsesQuadpoints: true,
    highlightApNoExtUsesQuadpoints: true,
    highlightApExtKeepsRawRect: true,
  },
...
console.error(
  `v3.0.14 annotation AP highlight quadpoints residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
