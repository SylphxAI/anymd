...
import { canonicalPopupAnnotationResidualResult, type Json } from './v3014-popup-annotation-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-popup-annotation-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-popup-annotation-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-popup-annotation-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-popup-annotation-residual-projection.ts');
const popupFixture = join(fixtureDir, 'v3014-annotation-popup-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') throw new Error('v3.0.14 popup annotation residual tag moved');
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) throw new Error('v3.0.14 popup annotation residual tree mismatch');
for (const [name, path, expected] of [['runner', runnerPath, baseline.runnerSha256], ['projection', projectionPath, baseline.projectionSha256], ['corpus', corpusPath, baseline.corpusSha256],   ['popupFixture', popupFixture, baseline.fixtureSha256['v3014-annotation-popup-v1.pdf']!],
  ['freeTextFixture', freeTextFixture, baseline.fixtureSha256['v3014-annotation-freetext-v1.pdf']!]] as const) {
  if (sha256(readFileSync(path)) !== expected) throw new Error(`popup annotation residual ${name} digest drift`);
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) throw new Error(`popup annotation residual entrypoint digest drift: ${path}`);
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) throw new Error('popup annotation residual bun.lock digest drift');
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-popup-annotation-residual-rust', version: '1' } });
    ...
    return canonicalPopupAnnotationResidualResult(await request(2, 'tools/call', { name: 'read_pdf', arguments: input }));
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-popup-annotation-relocated-'));
try {
  copyFileSync(popupFixture, join(relocatedDir, 'v3014-annotation-popup-v1.pdf'));
  copyFileSync(freeTextFixture, join(relocatedDir, 'v3014-annotation-freetext-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) throw new Error(`popup annotation residual comparator missed ${id}.${path.join('.')}`);
    leafMutationCount += 1;
  }
}
...
const popupGroup = (dataOf('popup-inherits-parent').annotations as Json[])[0] as Record<string, Json>;
const popupAnns = popupGroup.annotations as Json[];
const popup = popupAnns.find((a) => (a as Record<string, Json>).subtype === 'Popup') as Record<string, Json>;
const text = popupAnns.find((a) => (a as Record<string, Json>).subtype === 'Text') as Record<string, Json>;
if (popup.contents !== 'Parent note' || popup.title !== 'Author') throw new Error('popup must inherit parent contents/title');
if (text.contents !== 'Parent note') throw new Error('parent text contents mismatch');
const free = (((dataOf('freetext-no-parent-inherit').annotations as Json[])[0] as Record<string, Json>).annotations as Json[])[0] as Record<string, Json>;
if (free.subtype !== 'FreeText' || free.contents !== 'Hello FreeText') throw new Error('freetext control mismatch');
...
const report = {
  ...
  providerProof: { popupInheritsParentTitleContents: true, parentTextIconBox: true, freeTextNoParentInherit: true },
...
console.error(`v3.0.14 popup annotation residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`);
