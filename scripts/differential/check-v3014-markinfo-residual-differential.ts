...
const corpusPath = join(scriptDir, 'fixtures/v3014-markinfo-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-markinfo-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-markinfo-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-markinfo-residual-projection.ts');
const fixtures = {
  'v3014-markinfo-nonbool-v1.pdf': join(fixtureDir, 'v3014-markinfo-nonbool-v1.pdf'),
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 markinfo residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 markinfo residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ...Object.entries(fixtures).map(
    ([name, path]) => [name, path, baseline.fixtureSha256[name]!] as const
  ),
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`markinfo residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`markinfo residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('markinfo residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust markinfo residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-markinfo-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalMarkInfoResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-markinfo-residual-relocated-'));
try {
  for (const [name, path] of Object.entries(fixtures)) {
    copyFileSync(path, join(relocatedDir, name));
  }
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`markinfo residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const nonbool = oracle.expectations['markinfo-nonbool-and-missing'] as Record<string, Json>;
const alltrue = oracle.expectations['markinfo-all-true'] as Record<string, Json>;
const empty = oracle.expectations['markinfo-empty-all-false'] as Record<string, Json>;
const nonboolMark = (
  ((nonbool.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).mark_info as Record<string, Json>;
const alltrueMark = (
  ((alltrue.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).mark_info as Record<string, Json>;
const emptyMark = (
  ((empty.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).mark_info as Record<string, Json>;
if (
  nonboolMark.Marked !== true ||
  nonboolMark.UserProperties !== false ||
  nonboolMark.Suspects !== false
) {
  throw new Error('nonbool case must default missing/non-boolean MarkInfo keys to false');
}
if (
  alltrueMark.Marked !== true ||
  alltrueMark.UserProperties !== true ||
  alltrueMark.Suspects !== true
) {
  throw new Error('all-true case must keep all MarkInfo booleans true');
}
if (
  emptyMark.Marked !== false ||
  emptyMark.UserProperties !== false ||
  emptyMark.Suspects !== false
) {
  throw new Error('empty MarkInfo must project all false');
}
...
const report = {
  ...
  providerProof: {
    nonBooleanDefaultsFalse: true,
    allTruePreserved: true,
    emptyAllFalse: true,
  },
...
console.error(
  `v3.0.14 markinfo residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
