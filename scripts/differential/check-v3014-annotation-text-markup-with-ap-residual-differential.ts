...
import {
  canonicalAnnotationTextMarkupWithApResidualResult,
  type Json,
} from './v3014-annotation-text-markup-with-ap-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-text-markup-with-ap-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-text-markup-with-ap-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-text-markup-with-ap-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-text-markup-with-ap-residual-projection.ts');
const lineApAsMissingFixture = join(fixtureDir, 'v3014-annotation-underline-ap-keeps-rect-v1.pdf');
const lineApAsOnFixture = join(fixtureDir, 'v3014-annotation-squiggly-ap-keeps-rect-v1.pdf');
const lineApAsInvalidFixture = join(fixtureDir, 'v3014-annotation-strikeout-ap-keeps-rect-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP text-markup with-appearance residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP text-markup with-appearance residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    ...
    lineApAsMissingFixture,
    baseline.fixtureSha256['v3014-annotation-underline-ap-keeps-rect-v1.pdf']!,
  ],
  [
    'lineApAsOnFixture',
    lineApAsOnFixture,
    baseline.fixtureSha256['v3014-annotation-squiggly-ap-keeps-rect-v1.pdf']!,
  ],
  [
    'lineApAsInvalidFixture',
    lineApAsInvalidFixture,
    baseline.fixtureSha256['v3014-annotation-strikeout-ap-keeps-rect-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP text-markup with-appearance residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP text-markup with-appearance residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP text-markup with-appearance residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP text-markup with-appearance residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-text-markup-with-ap-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationTextMarkupWithApResidualResult(
...
try {
  copyFileSync(lineApAsMissingFixture, join(relocatedDir, 'v3014-annotation-underline-ap-keeps-rect-v1.pdf'));
  copyFileSync(lineApAsOnFixture, join(relocatedDir, 'v3014-annotation-squiggly-ap-keeps-rect-v1.pdf'));
  copyFileSync(lineApAsInvalidFixture, join(relocatedDir, 'v3014-annotation-strikeout-ap-keeps-rect-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP text-markup with-appearance residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const underline = ((
  dataOf('underline-ap-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const squiggly = ((
  dataOf('squiggly-ap-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const strikeout = ((
  dataOf('strikeout-ap-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  underlineBox.left !== 200 ||
  underlineBox.bottom !== 200 ||
  underlineBox.right !== 300 ||
  underlineBox.top !== 300
) {
  throw new Error('Underline with appearance must keep raw Rect');
}
if (
  squigglyBox.left !== 150 ||
  squigglyBox.bottom !== 160 ||
  squigglyBox.right !== 250 ||
  squigglyBox.top !== 260
) {
  throw new Error('Squiggly with appearance must keep raw Rect');
}
if (
  strikeoutBox.left !== 100 ||
  strikeoutBox.bottom !== 110 ||
  strikeoutBox.right !== 210 ||
  strikeoutBox.top !== 220
) {
  throw new Error('StrikeOut with appearance must keep raw Rect');
}
...
const report = {
  ...
  providerProof: {
    underlineApKeepsRawRect: true,
    squigglyApKeepsRawRect: true,
    strikeoutApKeepsRawRect: true,
  },
...
console.error(
  `v3.0.14 annotation AP text-markup with-appearance residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
