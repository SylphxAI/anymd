...
import {
  canonicalLinearizedResidualResult,
  type Json,
} from './v3014-linearized-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-linearized-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-linearized-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-linearized-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-linearized-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 linearized residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 linearized residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['validFixture', validFixture, baseline.fixtureSha256['v3014-info-linearized-valid-v1.pdf']!],
  ['spuriousFixture', spuriousFixture, baseline.fixtureSha256['v3014-info-linearized-spurious-v1.pdf']!],
  ['absentFixture', absentFixture, baseline.fixtureSha256['v3014-info-linearized-absent-v1.pdf']!],
] as const) {

  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`linearized residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`linearized residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('linearized residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust linearized residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-linearized-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalLinearizedResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-linearized-relocated-'));
try {
  copyFileSync(validFixture, join(relocatedDir, 'v3014-info-linearized-valid-v1.pdf'));
  copyFileSync(spuriousFixture, join(relocatedDir, 'v3014-info-linearized-spurious-v1.pdf'));
  copyFileSync(absentFixture, join(relocatedDir, 'v3014-info-linearized-absent-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`linearized residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const valid = dataOf('linearized-valid');
const spurious = dataOf('linearized-spurious');
const absent = dataOf('linearized-absent');
const validInfo = valid.info as Record<string, Json>;
const spuriousInfo = spurious.info as Record<string, Json>;
const absentInfo = absent.info as Record<string, Json>;
if (validInfo.IsLinearized !== true) {
  throw new Error('valid linearization residual must set IsLinearized=true');
}
if (spuriousInfo.IsLinearized !== false || absentInfo.IsLinearized !== false) {
  throw new Error('spurious/absent linearization residual must set IsLinearized=false');
}
if (valid.num_pages !== 1 || spurious.num_pages !== 1 || absent.num_pages !== 1) {
  throw new Error('all residual cases must keep top-level num_pages=1');
}
...
const report = {
  ...
  providerProof: {
    validLinearizedTrue: true,
    spuriousLinearizedFalse: true,
    absentLinearizedFalse: true,
    topLevelNumPagesPreserved: true,
  },
...
console.error(
  `v3.0.14 linearized residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
