...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  ...
  type Json,
} from './v3014-border-bs-nondict-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-border-bs-nondict-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-border-bs-nondict-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-border-bs-nondict-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-border-bs-nondict-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 border BS nondict residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 border BS nondict residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'polylineBorderBsNondictFixture',
    polylineBorderBsNondictFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-border-bs-null-v1.pdf']!,
  ],
  [
    'lineBorderBsNondictFixture',
    lineBorderBsNondictFixture,
    baseline.fixtureSha256['v3014-annotation-line-border-bs-null-v1.pdf']!,
  ],
  [
    'inkBorderBsNondictFixture',
    inkBorderBsNondictFixture,
    baseline.fixtureSha256['v3014-annotation-ink-border-bs-null-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`border BS nondict residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`border BS nondict residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('border BS nondict residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust border BS nondict residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-border-bs-nondict-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalBorderBsNondictResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-border-bs-nondict-relocated-'));
try {
  copyFileSync(polylineBorderBsNondictFixture, join(relocatedDir, 'v3014-annotation-polyline-border-bs-null-v1.pdf'));
  copyFileSync(lineBorderBsNondictFixture, join(relocatedDir, 'v3014-annotation-line-border-bs-null-v1.pdf'));
  copyFileSync(inkBorderBsNondictFixture, join(relocatedDir, 'v3014-annotation-ink-border-bs-null-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`border BS nondict residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const polyline = ((
  dataOf('polyline-border-bs-nondict-w1').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const line = ((
  dataOf('line-border-bs-nondict-w1').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const ink = ((
  dataOf('ink-border-bs-nondict-w1').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  ...
  throw new Error('PolyLine BS null over Border[2]=9 must keep default width 1 vertices expand by 2');
}
if (
  lineBox.left !== 7 ||
  lineBox.bottom !== 7 ||
  ...
  throw new Error('Line BS null over Border[2]=9 must keep default width 1 L expand then public +1');
}
if (
  inkBox.left !== 28 ||
  inkBox.bottom !== 28 ||
  ...
  throw new Error('Ink BS null over Border[2]=9 must keep default width 1 InkList expand by 2');
}
if ((polyline[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline border BS nondict residual subtype must remain PolyLine');
}
if ((line[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line border BS nondict residual subtype must remain Line');
}
if ((ink[0] as Record<string, Json>).subtype !== 'Ink') {
  throw new Error('ink border BS nondict residual subtype must remain Ink');
}
...
const report = {
  ...
  providerProof: {
    polylineBorderBsNondictW1: true,
    lineBorderBsNondictW1: true,
    inkBorderBsNondictW1: true,
  },
...
console.error(
  `v3.0.14 border BS nondict residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
