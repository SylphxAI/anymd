...
import {
  canonicalPageLabelsKidsResidualResult,
  type Json,
} from './v3014-page-labels-kids-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-page-labels-kids-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-page-labels-kids-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-page-labels-kids-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-page-labels-kids-residual-projection.ts');
const kidsFixture = join(fixtureDir, 'v3014-page-labels-kids-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 page labels kids residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 page labels kids residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['kidsFixture', kidsFixture, baseline.fixtureSha256['v3014-page-labels-kids-v1.pdf']!],
  ['multiFixture', multiFixture, baseline.fixtureSha256['v3014-page-labels-multi-v1.pdf']!],
  ['noneFixture', noneFixture, baseline.fixtureSha256['v3014-page-labels-none-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`page labels kids residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`page labels kids residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('page labels kids residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust page labels kids residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-page-labels-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalPageLabelsKidsResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-page-labels-relocated-'));
try {
  copyFileSync(kidsFixture, join(relocatedDir, 'v3014-page-labels-kids-v1.pdf'));
  copyFileSync(multiFixture, join(relocatedDir, 'v3014-page-labels-multi-v1.pdf'));
  copyFileSync(noneFixture, join(relocatedDir, 'v3014-page-labels-none-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`page labels kids residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const kids = dataOf('page-labels-kids-number-tree');
const multi = dataOf('page-labels-multi-style-control');
const absent = dataOf('page-labels-absent-control');
if (
  JSON.stringify(kids.page_labels) !== JSON.stringify(['i', 'ii', '10']) ||
  kids.num_pages !== 3
) {
  throw new Error('kids number-tree labels must be i/ii/10');
}
if (
  JSON.stringify(multi.page_labels) !== JSON.stringify(['P-i', '10', 'A']) ||
  multi.num_pages !== 3
) {
  throw new Error('multi-style control labels must be P-i/10/A');
}
if (Object.hasOwn(absent, 'page_labels') || absent.num_pages !== 1) {
  throw new Error('absent labels control must omit page_labels field');
}
...
const report = {
  ...
  providerProof: { pageLabelsKidsNumberTree: true, pageLabelsMultiStyleControl: true, pageLabelsAbsentControl: true },
...
console.error(
  `v3.0.14 page labels kids residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
