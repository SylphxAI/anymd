...
import { canonicalFormRadioBrokenParentChainResidualResult, type Json } from './v3014-form-radio-broken-parent-chain-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-broken-parent-chain-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-broken-parent-chain-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-broken-parent-chain-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-broken-parent-chain-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-broken-parent-chain residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-broken-parent-chain residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['brokenApStreamFixture', brokenApStreamFixture, baseline.fixtureSha256['v3014-form-radio-broken-parent-ap-stream-v1.pdf']!],
  ['brokenApnStreamFixture', brokenApnStreamFixture, baseline.fixtureSha256['v3014-form-radio-broken-parent-apn-stream-v1.pdf']!],
  ['brokenApNamedFixture', brokenApNamedFixture, baseline.fixtureSha256['v3014-form-radio-broken-parent-ap-named-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-broken-parent-chain residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-broken-parent-chain residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-broken-parent-chain residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-broken-parent-chain residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-broken-parent-chain-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioBrokenParentChainResidualResult(
...
try {
        copyFileSync(brokenApStreamFixture, join(relocatedDir, 'v3014-form-radio-broken-parent-ap-stream-v1.pdf'));
  copyFileSync(brokenApnStreamFixture, join(relocatedDir, 'v3014-form-radio-broken-parent-apn-stream-v1.pdf'));
  copyFileSync(brokenApNamedFixture, join(relocatedDir, 'v3014-form-radio-broken-parent-ap-named-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-broken-parent-chain residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const brokenApStream = fieldsOf('form-radio-broken-parent-ap-stream');
const brokenApnStream = fieldsOf('form-radio-broken-parent-apn-stream');
const brokenApNamed = fieldsOf('form-radio-broken-parent-ap-named');
const assertBrokenParent = (fields: Record<string, Json>[], label: string) => {
  if (fields.length !== 2) {
    throw new Error(`${label}: expected 2 form fields, got ${String(fields.length)}`);
  }
  const root = fields[0]!;
  const group = fields[1]!;
  if (root.name !== 'Plan' || root.type !== undefined || root.value !== undefined || root.id !== '7R') {
    throw new Error(`${label}: root stub must be name-only Plan/7R`);
  }
  if (group.name !== 'Opt' || group.type !== undefined || group.value !== undefined || group.id !== '8R') {
    throw new Error(`${label}: intermediate stub must be name-only Opt/8R (Parent-chain name, no widgets)`);
  }
};
assertBrokenParent(brokenApStream, 'broken parent AP stream');
assertBrokenParent(brokenApnStream, 'broken parent AP/N stream');
assertBrokenParent(brokenApNamed, 'broken parent named AP/N');
...
const report = {
  ...
  providerProof: {
    radioBrokenParentApStream: true,
    radioBrokenParentApnStream: true,
    radioBrokenParentApNamed: true,
  },
...
console.error(
  `v3.0.14 form radio-broken-parent-chain residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
