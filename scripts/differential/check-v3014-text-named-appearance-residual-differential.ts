...
import {
  canonicalTextNamedAppearanceResidualResult,
  type Json,
} from './v3014-text-named-appearance-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-text-named-appearance-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-text-named-appearance-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-text-named-appearance-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-text-named-appearance-residual-projection.ts');
const namedApFixture = join(fixtureDir, 'v3014-annotation-text-namedap-v1.pdf');
const namedNoAsFixture = join(fixtureDir, 'v3014-annotation-text-namedap-noas-v1.pdf');
...
for (const [name, path, expected] of [
  ...
  ['namedApFixture', namedApFixture, baseline.fixtureSha256['v3014-annotation-text-namedap-v1.pdf']!],
  ['namedNoAsFixture', namedNoAsFixture, baseline.fixtureSha256['v3014-annotation-text-namedap-noas-v1.pdf']!],
] as const) {
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-text-named-appearance-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalTextNamedAppearanceResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-text-named-appearance-relocated-'));
try {
  copyFileSync(namedApFixture, join(relocatedDir, 'v3014-annotation-text-namedap-v1.pdf'));
  copyFileSync(namedNoAsFixture, join(relocatedDir, 'v3014-annotation-text-namedap-noas-v1.pdf'));
  for (const entry of corpus.cases) {
...
const withAs = ((dataOf('named-appearance-with-as-raw-rect').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const noAs = ((dataOf('named-appearance-without-as-icon-box').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const withBox = (withAs[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const noAsBox = (noAs[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  withBox.left !== 80 ||
  withBox.bottom !== 600 ||
  withBox.right !== 180 ||
  ...
  throw new Error('Named appearance with AS must keep raw Rect geometry');
}
if (
  noAsBox.left !== 90 ||
  noAsBox.bottom !== 688 ||
  ...
  throw new Error('Named appearance without AS must use pdf.js 22x22 icon box');
}
if ((withAs[0] as Record<string, Json>).contents !== 'Named AP') {
  throw new Error('named-with-as contents mismatch');
}
if ((noAs[0] as Record<string, Json>).contents !== 'Named no AS') {
  throw new Error('named-without-as contents mismatch');
}
...
const report = {
  ...
  providerProof: { namedAppearanceWithAsKeepsRawRect: true, namedAppearanceWithoutAsUsesIconBox: true },
...
console.error(
  `v3.0.14 text named appearance residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
