...
const corpusPath = join(scriptDir, 'fixtures/v3014-outline-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-outline-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-outline-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-outline-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 outline residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 outline residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['uriChildFixture', uriChildFixture, baseline.fixtureSha256['v3014-outline-uri-child-v1.pdf']!],
  ['fithFixture', fithFixture, baseline.fixtureSha256['v3014-outline-fith-v1.pdf']!],
  ['noneFixture', noneFixture, baseline.fixtureSha256['v3014-outline-none-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`outline residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`outline residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('outline residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust outline residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-outline-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalOutlineResidualResult(
...
try {
  copyFileSync(uriChildFixture, join(relocatedDir, 'v3014-outline-uri-child-v1.pdf'));
  copyFileSync(fithFixture, join(relocatedDir, 'v3014-outline-fith-v1.pdf'));
  copyFileSync(noneFixture, join(relocatedDir, 'v3014-outline-none-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`outline residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const uri = dataOf('outline-uri-child-fit');
const fith = dataOf('outline-fith-coordinate');
const absent = dataOf('outline-absent');
const uriRoot = (uri.outline as Array<Record<string, Json>>)[0]!;
const uriChild = (uriRoot.items as Array<Record<string, Json>>)[0]!;
if (
  uriRoot.url !== 'https://example.com/docs' ||
  uriRoot.dest !== null ||
  uriRoot.bold !== true ||
  uriRoot.italic !== true ||
  JSON.stringify(uriRoot.color) !== JSON.stringify([64, 128, 191]) ||
  uriChild.title !== 'Page one' ||
  !Array.isArray(uriChild.dest) ||
  (uriChild.dest[1] as Record<string, Json>).name !== 'Fit'
) {
  throw new Error('URI parent must keep url/null dest/bold/italic/color with child Fit dest');
}
const fithRoot = (fith.outline as Array<Record<string, Json>>)[0]!;
if (
  !Array.isArray(fithRoot.dest) ||
  (fithRoot.dest[1] as Record<string, Json>).name !== 'FitH' ||
  fithRoot.dest[2] !== 700
) {
  throw new Error('FitH case must keep name FitH and coordinate 700');
}
if (Object.hasOwn(absent, 'outline') || absent.num_pages !== 1) {
  throw new Error('absent outline must omit outline field');
}
...
const report = {
  ...
  providerProof: {
    uriParentChildFit: true,
    fithCoordinate: true,
    absentOutlineOmitted: true,
  },
...
console.error(
  `v3.0.14 outline residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
