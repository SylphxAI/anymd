...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalAnnotationLinkUriNormalizeResidualResult,
  type Json,
} from './v3014-annotation-link-uri-normalize-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-link-uri-normalize-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-link-uri-normalize-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-link-uri-normalize-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-link-uri-normalize-residual-projection.ts');
const domainFixture = join(fixtureDir, 'v3014-annotation-link-uri-domain-v1.pdf');
const pathFixture = join(fixtureDir, 'v3014-annotation-link-uri-path-v1.pdf');
const wwwFixture = join(fixtureDir, 'v3014-annotation-link-uri-www-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP link-uri-normalize residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP link-uri-normalize residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'domainFixture',
    domainFixture,
    baseline.fixtureSha256['v3014-annotation-link-uri-domain-v1.pdf']!,
  ],
  [
    'pathFixture',
    pathFixture,
    baseline.fixtureSha256['v3014-annotation-link-uri-path-v1.pdf']!,
  ],
  [
    'wwwFixture',
    wwwFixture,
    baseline.fixtureSha256['v3014-annotation-link-uri-www-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP link-uri-normalize residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP link-uri-normalize residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP link-uri-normalize residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP link-uri-normalize residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-link-uri-normalize-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationLinkUriNormalizeResidualResult(
...
try {
  copyFileSync(domainFixture, join(relocatedDir, 'v3014-annotation-link-uri-domain-v1.pdf'));
  copyFileSync(pathFixture, join(relocatedDir, 'v3014-annotation-link-uri-path-v1.pdf'));
  copyFileSync(wwwFixture, join(relocatedDir, 'v3014-annotation-link-uri-www-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP link-uri-normalize residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const domain = ((
  dataOf('link-uri-domain').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const pathCase = ((
  dataOf('link-uri-path').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const www = ((
  dataOf('link-uri-www').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const assertLink = (
  annotations: Json[],
  url: string,
  ...
  if (b.left !== 72 || b.bottom !== 600 || b.right !== 120 || b.top !== 620) {
    throw new Error(`${label}: unexpected bbox ${JSON.stringify(b)}`);
  }
};
assertLink(domain, 'https://example.com/', 'domain trailing slash');
assertLink(pathCase, 'https://example.com/x', 'path preserved');
assertLink(www, 'http://www.example.com/', 'www default protocol');
...
console.error(
  `v3.0.14 annotation AP link-uri-normalize residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
