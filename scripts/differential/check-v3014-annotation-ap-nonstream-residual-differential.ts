...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalAnnotationApNonstreamResidualResult,
  type Json,
} from './v3014-annotation-ap-nonstream-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-ap-nonstream-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-ap-nonstream-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-ap-nonstream-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-ap-nonstream-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP non-stream residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP non-stream residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'polylineApNNullFixture',
    polylineApNNullFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-ap-n-name-v1.pdf']!,
  ],
  [
    'lineApNNullFixture',
    lineApNNullFixture,
    baseline.fixtureSha256['v3014-annotation-line-ap-n-null-v1.pdf']!,
  ],
  [
    'inkApNNullFixture',
    inkApNNullFixture,
    baseline.fixtureSha256['v3014-annotation-ink-ap-n-null-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP non-stream residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP non-stream residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP non-stream residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP non-stream residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-ap-nonstream-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationApNonstreamResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-annotation-ap-nonstream-relocated-'));
try {
  copyFileSync(polylineApNNullFixture, join(relocatedDir, 'v3014-annotation-polyline-ap-n-name-v1.pdf'));
  copyFileSync(lineApNNullFixture, join(relocatedDir, 'v3014-annotation-line-ap-n-null-v1.pdf'));
  copyFileSync(inkApNNullFixture, join(relocatedDir, 'v3014-annotation-ink-ap-n-null-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP non-stream residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const polyline = ((
  dataOf('polyline-ap-n-name-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const line = ((
  dataOf('line-ap-n-null-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const ink = ((
  dataOf('ink-ap-n-null-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  ...
  throw new Error('PolyLine AP/N name (non-stream) must expand vertices like no appearance');
}
if (
  lineBox.left !== 4 ||
  lineBox.bottom !== 4 ||
  ...
  throw new Error('Line AP/N null (non-stream) must expand L like no appearance');
}
if (
  inkBox.left !== 26 ||
  inkBox.bottom !== 26 ||
  ...
  throw new Error('Ink AP/N null (non-stream) must expand InkList like no appearance');
}
if ((polyline[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline annotation AP non-stream residual subtype must remain PolyLine');
}
if ((line[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line annotation AP non-stream residual subtype must remain Line');
}
if ((ink[0] as Record<string, Json>).subtype !== 'Ink') {
  throw new Error('ink annotation AP non-stream residual subtype must remain Ink');
}
...
const report = {
  ...
  providerProof: {
    polylineApNNameExpandsGeometry: true,
    lineApNNullExpandsGeometry: true,
    inkApNNullExpandsGeometry: true,
  },
...
console.error(
  `v3.0.14 annotation AP non-stream residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
