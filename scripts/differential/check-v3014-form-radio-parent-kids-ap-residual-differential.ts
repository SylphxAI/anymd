...
import { canonicalFormRadioParentKidsApResidualResult, type Json } from './v3014-form-radio-parent-kids-ap-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-parent-kids-ap-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-parent-kids-ap-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-parent-kids-ap-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-parent-kids-ap-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-parent-kids-ap residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-parent-kids-ap residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['kidsApStreamFixture', kidsApStreamFixture, baseline.fixtureSha256['v3014-form-radio-kids-ap-stream-v1.pdf']!],
  ['kidsApnStreamFixture', kidsApnStreamFixture, baseline.fixtureSha256['v3014-form-radio-kids-apn-stream-v1.pdf']!],
  ['kidsApNamedFixture', kidsApNamedFixture, baseline.fixtureSha256['v3014-form-radio-kids-ap-named-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-parent-kids-ap residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-parent-kids-ap residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-parent-kids-ap residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-parent-kids-ap residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-parent-kids-ap-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioParentKidsApResidualResult(
...
try {
        copyFileSync(kidsApStreamFixture, join(relocatedDir, 'v3014-form-radio-kids-ap-stream-v1.pdf'));
  copyFileSync(kidsApnStreamFixture, join(relocatedDir, 'v3014-form-radio-kids-apn-stream-v1.pdf'));
  copyFileSync(kidsApNamedFixture, join(relocatedDir, 'v3014-form-radio-kids-ap-named-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-parent-kids-ap residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const kidsApStream = fieldsOf('form-radio-kids-ap-stream');
const kidsApnStream = fieldsOf('form-radio-kids-apn-stream');
const kidsApNamed = fieldsOf('form-radio-kids-ap-named');
const assertKids = (fields: Record<string, Json>[], value: string, defaultValue: Json, label: string) => {
  if (fields.length !== 3) throw new Error(`${label}: expected 3 form fields, got ${String(fields.length)}`);
  const parent = fields[0]!;
  const kid0 = fields[1]!;
  const kid1 = fields[2]!;
  if (parent.name !== 'Plan' || parent.type !== undefined || parent.value !== undefined) {
    throw new Error(`${label}: parent stub must be name-only Plan`);
  }
  for (const [idx, kid] of [kid0, kid1].entries()) {
    if (kid.name !== 'Plan' || kid.type !== 'radiobutton' || kid.value !== value || kid.default_value !== defaultValue) {
      throw new Error(`${label}: kid[${String(idx)}] expected radiobutton value=${String(value)} default=${JSON.stringify(defaultValue)}`);
    }
  }
};
assertKids(kidsApStream, 'Gold', null, 'kids AP stream');
assertKids(kidsApnStream, 'Gold', null, 'kids AP/N stream');
assertKids(kidsApNamed, 'Gold', 'Off', 'kids named AP/N');
...
const report = {
  ...
  providerProof: {
    radioKidsApStream: true,
    radioKidsApnStream: true,
    radioKidsApNamed: true,
  },
...
console.error(
  `v3.0.14 form radio-parent-kids-ap residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
