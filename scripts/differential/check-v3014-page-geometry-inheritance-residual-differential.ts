...
import {
  canonicalPageGeometryInheritanceResidualResult,
  type Json,
} from './v3014-page-geometry-inheritance-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-page-geometry-inheritance-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-page-geometry-inheritance-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-page-geometry-inheritance-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-page-geometry-inheritance-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 page geometry inheritance residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 page geometry inheritance residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'inheritedRotateFixture',
    inheritedRotateFixture,
    baseline.fixtureSha256['v3014-page-geometry-inherited-rotate-v1.pdf']!,
  ],
  [
    'inheritedCropFixture',
    inheritedCropFixture,
    baseline.fixtureSha256['v3014-page-geometry-inherited-crop-v1.pdf']!,
  ],
  [
    'negativeRotateFixture',
    negativeRotateFixture,
    baseline.fixtureSha256['v3014-page-geometry-negative-rotate-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`page geometry inheritance residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`page geometry inheritance residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('page geometry inheritance residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust page geometry inheritance residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-page-geometry-inheritance-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalPageGeometryInheritanceResidualResult(
...
try {
  copyFileSync(inheritedRotateFixture, join(relocatedDir, 'v3014-page-geometry-inherited-rotate-v1.pdf'));
  copyFileSync(inheritedCropFixture, join(relocatedDir, 'v3014-page-geometry-inherited-crop-v1.pdf'));
  copyFileSync(negativeRotateFixture, join(relocatedDir, 'v3014-page-geometry-negative-rotate-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`page geometry inheritance residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const inheritedRotate = geomOf('page-geometry-inherited-rotate');
const inheritedCrop = geomOf('page-geometry-inherited-crop');
const negativeRotate = geomOf('page-geometry-negative-rotate');
if (
  inheritedRotate.width !== 200 ||
  inheritedRotate.height !== 300 ||
  inheritedRotate.rotation !== 180 ||
  inheritedRotate.user_unit !== 1
) {
  throw new Error('inherited MediaBox+Rotate must project 200x300 at rotation 180');
}
if (
  inheritedCrop.width !== 200 ||
  inheritedCrop.height !== 300 ||
  inheritedCrop.rotation !== 0 ||
  (inheritedCrop.view_box as Record<string, Json>).left !== 10 ||
  (inheritedCrop.view_box as Record<string, Json>).bottom !== 20 ||
  (inheritedCrop.view_box as Record<string, Json>).right !== 210 ||
  (inheritedCrop.view_box as Record<string, Json>).top !== 320
) {
  throw new Error('inherited MediaBox with page CropBox must intersect to crop view_box 10,20,210,320');
}
if (
  negativeRotate.width !== 300 ||
  negativeRotate.height !== 200 ||
  negativeRotate.rotation !== 270
) {
  throw new Error('Rotate -90 must normalize to 270 and swap width/height');
}
...
const report = {
  ...
  providerProof: {
    inheritedMediaBoxRotate: true,
    inheritedMediaBoxPageCrop: true,
    negativeRightAngleRotate: true,
  },
...
console.error(
  `v3.0.14 page geometry inheritance residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
