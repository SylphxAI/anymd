...
import {
  canonicalPageLabelsResidualResult,
  type Json,
} from './v3014-page-labels-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-page-labels-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-page-labels-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-page-labels-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-page-labels-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 page labels residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 page labels residual tree mismatch');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust page labels residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    ...
    return canonicalPageLabelsResidualResult(
...
try {
  ...
  copyFileSync(prefixFixture, join(relocatedDir, 'v3014-page-labels-prefix-v1.pdf'));
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`page labels residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const multi = dataOf('page-labels-multi-style');
const prefix = dataOf('page-labels-prefix-start');
const absent = dataOf('page-labels-absent');
if (
  JSON.stringify(multi.page_labels) !== JSON.stringify(['P-i', '10', 'A']) ||
  multi.num_pages !== 3
) {
  throw new Error('multi-style labels must be P-i/10/A');
}
if (
  JSON.stringify(prefix.page_labels) !== JSON.stringify(['Ch-3', 'Ch-4']) ||
  prefix.num_pages !== 2
) {
  throw new Error('prefix-start labels must be Ch-3/Ch-4');
}
if (Object.hasOwn(absent, 'page_labels') || absent.num_pages !== 1) {
  throw new Error('absent labels must omit page_labels field');
}
...
const report = {
  ...
  providerProof: {
    multiStyleLabels: true,
    prefixStartDecimal: true,
    absentLabelsOmitted: true,
  },
...
console.error(
  `v3.0.14 page labels residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
