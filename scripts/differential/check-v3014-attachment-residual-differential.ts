...
import {
  canonicalAttachmentResidualResult,
  type Json,
} from './v3014-attachment-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-attachment-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-attachment-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-attachment-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 attachment residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 attachment residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['kidsFixture', kidsFixture, baseline.fixtureSha256['v3014-attachment-kids-v1.pdf']!],
  [
    'filenameFixture',
    filenameFixture,
    baseline.fixtureSha256['v3014-attachment-filename-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`attachment residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`attachment residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('attachment residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust attachment residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-attachment-residual-rust', version: '1' },
    });
    child.stdin.write(
...
try {
  copyFileSync(kidsFixture, join(relocatedDir, 'v3014-attachment-kids-v1.pdf'));
  copyFileSync(filenameFixture, join(relocatedDir, 'v3014-attachment-filename-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`attachment residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const kids = oracle.expectations['attachment-name-tree-kids-win'] as Record<string, Json>;
const names = oracle.expectations['attachment-filename-normalization'] as Record<string, Json>;
const kidsAtt = (
  ((kids.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).attachments as Array<Record<string, Json>>;
const nameAtt = (
  ((names.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).attachments as Array<Record<string, Json>>;
if (kidsAtt.length !== 1 || kidsAtt[0]!.name !== 'child' || kidsAtt[0]!.filename !== 'a.txt') {
  throw new Error('kids-win case must expose only child/a.txt');
}
if (nameAtt.length !== 2) throw new Error('filename case must expose two attachments');
if (nameAtt[0]!.filename !== 'unnamed' || nameAtt[1]!.filename !== 'report.txt') {
  throw new Error('filename normalization must map slash->unnamed and windows path basename');
}
...
const report = {
  ...
  providerProof: {
    nameTreeKidsWinOverNames: true,
    trailingSlashUnnamed: true,
    windowsPathBasename: true,
  },
...
console.error(
  `v3.0.14 attachment residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
