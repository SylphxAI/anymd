...
import {
  canonicalInfoExtrasResidualResult,
  type Json,
} from './v3014-info-extras-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-info-extras-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-info-extras-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-info-extras-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-info-extras-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 info extras residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 info extras residual tree mismatch');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust info extras residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-info-extras-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalInfoExtrasResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-info-extras-relocated-'));
try {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`info extras residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const acroform = dataOf('info-extras-acroform-lang');
const plain = dataOf('info-extras-plain');
const forbidden = ['text_chars', 'route', 'num_pages'];
for (const [id, data] of [
  ['info-extras-acroform-lang', acroform],
  ['info-extras-plain', plain],
] as const) {
  const keys = data.info_keys as string[];
  if (!Array.isArray(keys) || keys.some((key) => forbidden.includes(key))) {
    throw new Error(`${id} oracle must omit rust-only info extras`);
  }
  if (data.has_info !== true) {
    throw new Error(`${id} must keep has_info`);
  }
  if (data.num_pages !== 1) {
    throw new Error(`${id} must keep top-level num_pages`);
  }
}
...
const report = {
  ...
  providerProof: {
    noRustOnlyInfoExtras: true,
    exactInfoKeySet: true,
...
console.error(
  `v3.0.14 info extras residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
