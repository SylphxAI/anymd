...
import {
  canonicalTextAnnotationResidualResult,
  type Json,
} from './v3014-text-annotation-residual-projection.ts';
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 text annotation residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 text annotation residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['textNoApFixture', textNoApFixture, baseline.fixtureSha256['v3014-annotation-text-noap-v1.pdf']!],
  ['freeTextFixture', freeTextFixture, baseline.fixtureSha256['v3014-annotation-freetext-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`text annotation residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`text annotation residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('text annotation residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust text annotation residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-text-annotation-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalTextAnnotationResidualResult(
...
try {
  copyFileSync(textNoApFixture, join(relocatedDir, 'v3014-annotation-text-noap-v1.pdf'));
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`text annotation residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const textNoAp = ((dataOf('text-no-appearance-icon-box').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const freeText = ((dataOf('freetext-raw-rect').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const textBox = (textNoAp[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const freeBox = (freeText[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  textBox.left !== 120 ||
  textBox.bottom !== 678 ||
  textBox.right !== 142 ||
  ...
  throw new Error('Text no-appearance residual must use pdf.js 22x22 icon box');
}
if (
  freeBox.left !== 100 ||
  freeBox.bottom !== 600 ||
  ...
  throw new Error('FreeText residual must keep raw Rect geometry');
}
...
const report = {
  ...
  providerProof: {
    textNoAppearanceIconBox: true,
    freeTextRawRect: true,
  },
...
console.error(
  `v3.0.14 text annotation residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
