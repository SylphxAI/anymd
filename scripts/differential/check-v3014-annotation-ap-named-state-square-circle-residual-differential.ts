...
import {
  canonicalAnnotationApNamedStateSquareCircleResidualResult,
  type Json,
} from './v3014-annotation-ap-named-state-square-circle-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-square-circle-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-square-circle-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-ap-named-state-square-circle-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-ap-named-state-square-circle-residual-projection.ts');
const lineApAsMissingFixture = join(fixtureDir, 'v3014-annotation-circle-ap-as-missing-v1.pdf');
const lineApAsOnFixture = join(fixtureDir, 'v3014-annotation-square-ap-as-on-v1.pdf');
const lineApAsInvalidFixture = join(fixtureDir, 'v3014-annotation-square-ap-as-invalid-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP named-state square/circle residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP named-state square/circle residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    ...
    lineApAsMissingFixture,
    baseline.fixtureSha256['v3014-annotation-circle-ap-as-missing-v1.pdf']!,
  ],
  [
    'lineApAsOnFixture',
    lineApAsOnFixture,
    baseline.fixtureSha256['v3014-annotation-square-ap-as-on-v1.pdf']!,
  ],
  [
    'lineApAsInvalidFixture',
    lineApAsInvalidFixture,
    baseline.fixtureSha256['v3014-annotation-square-ap-as-invalid-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP named-state square/circle residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP named-state square/circle residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP named-state square/circle residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP named-state square/circle residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-ap-named-state-square-circle-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationApNamedStateSquareCircleResidualResult(
...
try {
  copyFileSync(lineApAsMissingFixture, join(relocatedDir, 'v3014-annotation-circle-ap-as-missing-v1.pdf'));
  copyFileSync(lineApAsOnFixture, join(relocatedDir, 'v3014-annotation-square-ap-as-on-v1.pdf'));
  copyFileSync(lineApAsInvalidFixture, join(relocatedDir, 'v3014-annotation-square-ap-as-invalid-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP named-state square/circle residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const asOn = ((
  dataOf('square-ap-as-on-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asMissing = ((
  dataOf('circle-ap-as-missing-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asInvalid = ((
  dataOf('square-ap-as-invalid-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asOnBox = (asOn[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const asMissingBox = (asMissing[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const asInvalidBox = (asInvalid[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  asOnBox.left !== 200 ||
  asOnBox.bottom !== 200 ||
  asOnBox.right !== 300 ||
  ...
  throw new Error('Square AP/N named-state with AS/On must keep raw Rect');
}
if (
  asMissingBox.left !== 50 ||
  asMissingBox.bottom !== 60 ||
  asMissingBox.right !== 150 ||
  asMissingBox.top !== 160
) {
  throw new Error('Circle AP/N named-state without AS must keep raw Rect');
}
if (
  asInvalidBox.left !== 10 ||
  asInvalidBox.bottom !== 20 ||
  asInvalidBox.right !== 110 ||
  asInvalidBox.top !== 120
) {
  throw new Error('Square AP/N named-state with invalid AS must keep raw Rect');
}
if ((asOn[0] as Record<string, Json>).subtype !== 'Square') {
  throw new Error('square AP named-state residual subtype must remain Square');
}
if ((asMissing[0] as Record<string, Json>).subtype !== 'Circle') {
  throw new Error('circle AP named-state residual subtype must remain Circle');
}
if ((asInvalid[0] as Record<string, Json>).subtype !== 'Square') {
  throw new Error('square AP named-state residual subtype must remain Square');
}
...
const report = {
  ...
  providerProof: {
    squareApAsOnKeepsRawRect: true,
    circleApAsMissingKeepsRawRect: true,
    squareApAsInvalidKeepsRawRect: true,
  },
...
console.error(
  `v3.0.14 annotation AP named-state square/circle residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
