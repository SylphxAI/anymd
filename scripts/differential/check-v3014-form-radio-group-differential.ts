...
import { canonicalFormRadioGroupResult, type Json } from './v3014-form-radio-group-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-group-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-group-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-group-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-group-projection.ts');
const groupFixture = join(fixtureDir, 'v3014-form-radio-group-v1.pdf');
const threeFixture = join(fixtureDir, 'v3014-form-radio-group-three-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-group tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-group tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['groupFixture', groupFixture, baseline.fixtureSha256['v3014-form-radio-group-v1.pdf']!],
  ['threeFixture', threeFixture, baseline.fixtureSha256['v3014-form-radio-group-three-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-group ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-group entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-group bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-group timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-group-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioGroupResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-form-radio-group-relocated-'));
try {
  copyFileSync(groupFixture, join(relocatedDir, 'v3014-form-radio-group-v1.pdf'));
  copyFileSync(threeFixture, join(relocatedDir, 'v3014-form-radio-group-three-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-group comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const group = oracle.expectations['form-radio-group-kids-with-checkbox'] as Record<string, Json>;
const three = oracle.expectations['form-radio-group-three-options'] as Record<string, Json>;
const groupFields = (
  ((group.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
const threeFields = (
  ((three.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
if (groupFields.length !== 4 || groupFields[0]!.name !== 'tier' || Object.hasOwn(groupFields[0]!, 'type')) {
  throw new Error('group case must start with parent stub name-only tier field');
}
if (
  groupFields.filter((f) => f.type === 'radiobutton').length !== 2 ||
  groupFields.some((f) => f.type === 'radiobutton' && f.value !== 'Gold')
) {
  throw new Error('group kids must be radiobuttons inheriting value Gold');
}
if (threeFields.length !== 4 || threeFields[0]!.name !== 'color') {
  throw new Error('three-option case must expose parent stub plus three kids');
}
if (
  threeFields.filter((f) => f.type === 'radiobutton').length !== 3 ||
  threeFields.some(
    (f) => f.type === 'radiobutton' && (f.value !== 'Silver' || f.default_value !== 'Bronze')
  )
) {
  throw new Error('three-option kids must inherit value Silver and default Bronze');
}
...
const report = {
  ...
  providerProof: {
    parentStubPlusRadiobuttonKids: true,
    inheritedValueAndDefault: true,
    threeOptionRadioGroup: true,
  },
...
console.error(
  `v3.0.14 form radio-group differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
