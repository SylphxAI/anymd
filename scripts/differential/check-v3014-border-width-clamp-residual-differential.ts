...
import {
  canonicalBorderWidthClampResidualResult,
  type Json,
} from './v3014-border-width-clamp-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-border-width-clamp-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-border-width-clamp-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-border-width-clamp-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-border-width-clamp-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 border-width clamp residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 border-width clamp residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'polylineClampFixture',
    polylineClampFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-clamp-w2-v1.pdf']!,
  ],
  [
    'lineClampFixture',
    lineClampFixture,
    baseline.fixtureSha256['v3014-annotation-line-clamp-w2-v1.pdf']!,
  ],
  [
    'inkClampFixture',
    inkClampFixture,
    baseline.fixtureSha256['v3014-annotation-ink-clamp-w2-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`border-width clamp residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`border-width clamp residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('border-width clamp residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust border-width clamp residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-border-width-clamp-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalBorderWidthClampResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-border-width-clamp-relocated-'));
try {
  copyFileSync(polylineClampFixture, join(relocatedDir, 'v3014-annotation-polyline-clamp-w2-v1.pdf'));
  copyFileSync(lineClampFixture, join(relocatedDir, 'v3014-annotation-line-clamp-w2-v1.pdf'));
  copyFileSync(inkClampFixture, join(relocatedDir, 'v3014-annotation-ink-clamp-w2-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`border-width clamp residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const polyline = ((
  dataOf('polyline-tiny-rect-clamps-border-width').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const line = ((
  dataOf('line-tiny-rect-clamps-border-width').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const ink = ((
  dataOf('ink-tiny-rect-clamps-border-width').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  ...
  throw new Error('tiny-Rect PolyLine BS/W=2 must clamp borderWidth to 1 before vertices expand');
}
if (
  lineBox.left !== 7 ||
  lineBox.bottom !== 7 ||
  ...
  throw new Error('tiny-Rect Line BS/W=2 must clamp borderWidth to 1 before L expand');
}
if (
  inkBox.left !== 28 ||
  inkBox.bottom !== 28 ||
  ...
  throw new Error('tiny-Rect Ink BS/W=2 must clamp borderWidth to 1 before InkList expand');
}
if ((polyline[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline clamp residual subtype must remain PolyLine');
}
if ((line[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line clamp residual subtype must remain Line');
}
if ((ink[0] as Record<string, Json>).subtype !== 'Ink') {
  throw new Error('ink clamp residual subtype must remain Ink');
}
...
const report = {
  ...
  providerProof: {
    polylineTinyRectClampsBorderWidth: true,
    lineTinyRectClampsBorderWidth: true,
    inkTinyRectClampsBorderWidth: true,
  },
...
console.error(
  `v3.0.14 border-width clamp residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
