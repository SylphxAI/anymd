...
const corpusPath = join(scriptDir, 'fixtures/v3014-info-trapped-custom-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-info-trapped-custom-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-info-trapped-custom-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-info-trapped-custom-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 info trapped-custom residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 info trapped-custom residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['trappedTrueFixture', trappedTrueFixture, baseline.fixtureSha256['v3014-info-trapped-true-v1.pdf']!],
  ['trappedFalseFixture', trappedFalseFixture, baseline.fixtureSha256['v3014-info-trapped-false-v1.pdf']!],
  ['customMixedFixture', customMixedFixture, baseline.fixtureSha256['v3014-info-custom-mixed-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`info trapped-custom residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`info trapped-custom residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('info trapped-custom residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust info trapped-custom residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-info-trapped-custom-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalInfoTrappedCustomResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-info-trapped-custom-relocated-'));
try {
  copyFileSync(trappedTrueFixture, join(relocatedDir, 'v3014-info-trapped-true-v1.pdf'));
  copyFileSync(trappedFalseFixture, join(relocatedDir, 'v3014-info-trapped-false-v1.pdf'));
  copyFileSync(customMixedFixture, join(relocatedDir, 'v3014-info-custom-mixed-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`info trapped-custom residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const trappedTrue = dataOf('info-trapped-true').info as Record<string, Json>;
const trappedFalse = dataOf('info-trapped-false').info as Record<string, Json>;
const customMixed = dataOf('info-custom-mixed').info as Record<string, Json>;
if (
  !trappedTrue.Trapped ||
  typeof trappedTrue.Trapped !== 'object' ||
  (trappedTrue.Trapped as Record<string, Json>).name !== 'True'
) {
  throw new Error('Trapped /True must project as {name:"True"}');
}
if (
  !trappedFalse.Trapped ||
  typeof trappedFalse.Trapped !== 'object' ||
  (trappedFalse.Trapped as Record<string, Json>).name !== 'False'
) {
  throw new Error('Trapped /False must project as {name:"False"}');
}
const custom = customMixed.Custom as Record<string, Json>;
if (
  !custom ||
  custom.Note !== 'hello' ||
  custom.Count !== 3 ||
  custom.Ok !== true ||
  !custom.Dept ||
  typeof custom.Dept !== 'object' ||
  (custom.Dept as Record<string, Json>).name !== 'Engineering'
) {
  throw new Error('Custom must project string/number/boolean/Name values');
}
...
const report = {
  ...
  providerProof: {
    trappedTrueName: true,
    trappedFalseName: true,
    customMixedValues: true,
  },
...
console.error(
  `v3.0.14 info trapped-custom residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
