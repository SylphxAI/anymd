...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalUtf16TextResidualResult,
  type Json,
} from './v3014-utf16-text-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-utf16-text-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-utf16-text-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-utf16-text-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-utf16-text-residual-projection.ts');
const annotFixture = join(fixtureDir, 'v3014-annot-utf16-odd-v1.pdf');
const freetextFixture = join(fixtureDir, 'v3014-freetext-utf16-odd-v1.pdf');
const catalogFixture = join(fixtureDir, 'v3014-catalog-utf16-odd-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 utf16-text residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 utf16-text residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['annotFixture', annotFixture, baseline.fixtureSha256['v3014-annot-utf16-odd-v1.pdf']!],
  ['freetextFixture', freetextFixture, baseline.fixtureSha256['v3014-freetext-utf16-odd-v1.pdf']!],
  ['catalogFixture', catalogFixture, baseline.fixtureSha256['v3014-catalog-utf16-odd-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`utf16-text residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`utf16-text residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('utf16-text residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust utf16-text residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-utf16-text-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalUtf16TextResidualResult(
...
try {
  copyFileSync(annotFixture, join(relocatedDir, 'v3014-annot-utf16-odd-v1.pdf'));
  copyFileSync(freetextFixture, join(relocatedDir, 'v3014-freetext-utf16-odd-v1.pdf'));
  copyFileSync(catalogFixture, join(relocatedDir, 'v3014-catalog-utf16-odd-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`utf16-text residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const annot = ((dataOf('annot-text-utf16-odd').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Array<Record<string, Json>>;
const freetext = ((dataOf('freetext-utf16-odd').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Array<Record<string, Json>>;
const catalog = dataOf('catalog-utf16-odd');
const outline = catalog.outline as Array<Record<string, Json>>;
const info = catalog.info as Record<string, Json>;
if (annot[0]?.contents !== 'Bo' || annot[0]?.title !== 'Aut') {
  throw new Error('Text annotation odd UTF-16 must decode contents=Bo title=Aut');
}
if (freetext[0]?.contents !== 'Hi') {
  throw new Error('FreeText odd UTF-16 must decode contents=Hi');
}
if (info.Title !== 'Bo' || info.Author !== 'Ada') {
  throw new Error('catalog info odd UTF-16 must decode Title=Bo Author=Ada');
}
if (outline[0]?.title !== 'Ch') {
  throw new Error('outline odd UTF-16 must decode title=Ch');
}
...
const report = {
  ...
  providerProof: {
    annotTextOddUtf16: true,
    freeTextOddUtf16: true,
    catalogOutlineAndInfoOddUtf16: true,
  },
  nonclaims: corpus.nonclaims,
  productTruth: { dropInFor3014: false, publishFreeze: true },
  capabilityStatus: { includeAnnotations: 'PARTIAL', includeOutline: 'PARTIAL', includeMetadata: 'PARTIAL' },
...
console.error(
  `v3.0.14 utf16-text residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
