...
import { canonicalFormRadioDeeperKidsApResidualResult, type Json } from './v3014-form-radio-deeper-kids-ap-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-deeper-kids-ap-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-deeper-kids-ap-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-deeper-kids-ap-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-deeper-kids-ap-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-deeper-kids-ap residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-deeper-kids-ap residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['deeperApStreamFixture', deeperApStreamFixture, baseline.fixtureSha256['v3014-form-radio-deeper-kids-ap-stream-v1.pdf']!],
  ['deeperApnStreamFixture', deeperApnStreamFixture, baseline.fixtureSha256['v3014-form-radio-deeper-kids-apn-stream-v1.pdf']!],
  ['deeperApNamedFixture', deeperApNamedFixture, baseline.fixtureSha256['v3014-form-radio-deeper-kids-ap-named-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-deeper-kids-ap residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-deeper-kids-ap residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-deeper-kids-ap residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-deeper-kids-ap residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-deeper-kids-ap-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioDeeperKidsApResidualResult(
...
try {
        copyFileSync(deeperApStreamFixture, join(relocatedDir, 'v3014-form-radio-deeper-kids-ap-stream-v1.pdf'));
  copyFileSync(deeperApnStreamFixture, join(relocatedDir, 'v3014-form-radio-deeper-kids-apn-stream-v1.pdf'));
  copyFileSync(deeperApNamedFixture, join(relocatedDir, 'v3014-form-radio-deeper-kids-ap-named-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-deeper-kids-ap residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const deeperApStream = fieldsOf('form-radio-deeper-kids-ap-stream');
const deeperApnStream = fieldsOf('form-radio-deeper-kids-apn-stream');
const deeperApNamed = fieldsOf('form-radio-deeper-kids-ap-named');
const assertDeeperKids = (
  ...
  if (fields.length !== 4) {
    throw new Error(`${label}: expected 4 form fields, got ${String(fields.length)}`);
  }
  const root = fields[0]!;
  const group = fields[1]!;
  const leaf0 = fields[2]!;
  const leaf1 = fields[3]!;
  if (root.name !== 'Plan' || root.type !== undefined || root.value !== undefined) {
    throw new Error(`${label}: root stub must be name-only Plan`);
  }
  if (group.name !== 'Plan.Opt' || group.type !== undefined || group.value !== undefined) {
    throw new Error(`${label}: intermediate stub must be name-only Plan.Opt`);
  }
  for (const [idx, leaf] of [leaf0, leaf1].entries()) {
    if (
      leaf.name !== 'Plan.Opt' ||
      leaf.type !== 'radiobutton' ||
      leaf.value !== value ||
      leaf.default_value !== defaultValue
    ) {
      throw new Error(
        `${label}: leaf[${String(idx)}] expected radiobutton Plan.Opt value=${String(value)} default=${JSON.stringify(defaultValue)}`
      );
    }
  }
};
assertDeeperKids(deeperApStream, 'Gold', null, 'deeper kids AP stream');
assertDeeperKids(deeperApnStream, 'Gold', null, 'deeper kids AP/N stream');
assertDeeperKids(deeperApNamed, 'Gold', 'Off', 'deeper kids named AP/N');
...
const report = {
  ...
  providerProof: {
    radioDeeperKidsApStream: true,
    radioDeeperKidsApnStream: true,
    radioDeeperKidsApNamed: true,
  },
...
console.error(
  `v3.0.14 form radio-deeper-kids-ap residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
