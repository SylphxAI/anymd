...
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-ap-named-state-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-ap-named-state-residual-projection.ts');
const lineApAsMissingFixture = join(fixtureDir, 'v3014-annotation-line-ap-as-missing-v1.pdf');
const lineApAsOnFixture = join(fixtureDir, 'v3014-annotation-line-ap-as-on-v1.pdf');
const lineApAsInvalidFixture = join(fixtureDir, 'v3014-annotation-line-ap-as-invalid-v1.pdf');
...
for (const [name, path, expected] of [
  ...
  [
    ...
    lineApAsMissingFixture,
    baseline.fixtureSha256['v3014-annotation-line-ap-as-missing-v1.pdf']!,
  ],
  [
    'lineApAsOnFixture',
    lineApAsOnFixture,
    baseline.fixtureSha256['v3014-annotation-line-ap-as-on-v1.pdf']!,
  ],
  [
    'lineApAsInvalidFixture',
    lineApAsInvalidFixture,
    baseline.fixtureSha256['v3014-annotation-line-ap-as-invalid-v1.pdf']!,
  ],
] as const) {
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-ap-named-state-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationApNamedStateResidualResult(
...
try {
  copyFileSync(lineApAsMissingFixture, join(relocatedDir, 'v3014-annotation-line-ap-as-missing-v1.pdf'));
  copyFileSync(lineApAsOnFixture, join(relocatedDir, 'v3014-annotation-line-ap-as-on-v1.pdf'));
  copyFileSync(lineApAsInvalidFixture, join(relocatedDir, 'v3014-annotation-line-ap-as-invalid-v1.pdf'));
  for (const entry of corpus.cases) {
...
const asOn = ((
  dataOf('line-ap-as-on-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asMissing = ((
  dataOf('line-ap-as-missing-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asInvalid = ((
  dataOf('line-ap-as-invalid-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  ...
  throw new Error('Line AP/N named-state with AS/On must keep raw Rect');
}
if (
  asMissingBox.left !== 4 ||
  asMissingBox.bottom !== 4 ||
  asMissingBox.right !== 106 ||
  asMissingBox.top !== 86
) {
  throw new Error('Line AP/N named-state without AS must expand L');
}
if (
  asInvalidBox.left !== 4 ||
  asInvalidBox.bottom !== 4 ||
  asInvalidBox.right !== 106 ||
  asInvalidBox.top !== 86
) {
  throw new Error('Line AP/N named-state with invalid AS must expand L');
}
if ((asOn[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line AP named-state residual subtype must remain Line');
}
if ((asMissing[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line AP named-state residual subtype must remain Line');
}
if ((asInvalid[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line AP named-state residual subtype must remain Line');
}
...
const report = {
  ...
  providerProof: {
    lineApAsOnKeepsRawRect: true,
    lineApAsMissingExpandsGeometry: true,
    lineApAsInvalidExpandsGeometry: true,
  },
...
console.error(
  `v3.0.14 annotation AP named-state residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
