...
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-malformed-ap-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-malformed-ap-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-malformed-ap-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-malformed-ap-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-malformed-ap residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-malformed-ap residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['apStreamFixture', apStreamFixture, baseline.fixtureSha256['v3014-form-checkbox-ap-stream-keeps-v-v1.pdf']!],
  ['apnStreamFixture', apnStreamFixture, baseline.fixtureSha256['v3014-form-checkbox-apn-stream-keeps-v-v1.pdf']!],
  ['apNamedFixture', apNamedFixture, baseline.fixtureSha256['v3014-form-checkbox-ap-named-as-off-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-malformed-ap residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-malformed-ap residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-malformed-ap residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-malformed-ap residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-malformed-ap-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxMalformedApResidualResult(
...
try {
        copyFileSync(apStreamFixture, join(relocatedDir, 'v3014-form-checkbox-ap-stream-keeps-v-v1.pdf'));
  copyFileSync(apnStreamFixture, join(relocatedDir, 'v3014-form-checkbox-apn-stream-keeps-v-v1.pdf'));
  copyFileSync(apNamedFixture, join(relocatedDir, 'v3014-form-checkbox-ap-named-as-off-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-malformed-ap residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const apStream = fieldsOf('form-checkbox-ap-stream-keeps-v')[0]!;
const apnStream = fieldsOf('form-checkbox-apn-stream-keeps-v')[0]!;
const apNamed = fieldsOf('form-checkbox-ap-named-as-off')[0]!;
if (apStream.type !== 'checkbox' || apStream.value !== 'Yes' || apStream.default_value !== null) {
  throw new Error('checkbox AP stream must keep V=/Yes and null default');
}
if (apnStream.type !== 'checkbox' || apnStream.value !== 'Yes' || apnStream.default_value !== null) {
  throw new Error('checkbox AP/N stream must keep V=/Yes and null default');
}
if (apNamed.type !== 'checkbox' || apNamed.value !== 'Off' || apNamed.default_value !== 'Off') {
  throw new Error('checkbox named AP/N AS=/Off must override V and default Off');
}
...
const report = {
  ...
  providerProof: {
    checkboxApStreamKeepsV: true,
    checkboxApnStreamKeepsV: true,
    checkboxApNamedAsOff: true,
  },
...
console.error(
  `v3.0.14 form checkbox-malformed-ap residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
