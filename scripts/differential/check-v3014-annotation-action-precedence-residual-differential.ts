...
import {
  canonicalAnnotationActionPrecedenceResidualResult,
  type Json,
} from './v3014-annotation-action-precedence-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-action-precedence-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-action-precedence-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-action-precedence-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-action-precedence-residual-projection.ts');
const gotoOverDestFixture = join(fixtureDir, 'v3014-annotation-goto-over-dest-v1.pdf');
const uriOverDestFixture = join(fixtureDir, 'v3014-annotation-uri-over-dest-v1.pdf');
const launchFileFixture = join(fixtureDir, 'v3014-annotation-launch-file-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation action precedence residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation action precedence residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'gotoOverDestFixture',
    gotoOverDestFixture,
    baseline.fixtureSha256['v3014-annotation-goto-over-dest-v1.pdf']!,
  ],
  [
    'uriOverDestFixture',
    uriOverDestFixture,
    baseline.fixtureSha256['v3014-annotation-uri-over-dest-v1.pdf']!,
  ],
  [
    'launchFileFixture',
    launchFileFixture,
    baseline.fixtureSha256['v3014-annotation-launch-file-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation action precedence residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation action precedence residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation action precedence residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(
          new Error(`Rust annotation action precedence residual timed out: ${stderr.slice(-2000)}`)
        );
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-action-precedence-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationActionPrecedenceResidualResult(
...
const relocatedDir = mkdtempSync(
  join(tmpdir(), 'pdf-reader-annotation-action-precedence-relocated-')
);
try {
  copyFileSync(gotoOverDestFixture, join(relocatedDir, 'v3014-annotation-goto-over-dest-v1.pdf'));
  copyFileSync(uriOverDestFixture, join(relocatedDir, 'v3014-annotation-uri-over-dest-v1.pdf'));
  copyFileSync(launchFileFixture, join(relocatedDir, 'v3014-annotation-launch-file-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(
        `annotation action precedence residual comparator missed ${id}.${path.join('.')}`
      );
    }
    leafMutationCount += 1;
  }
...
const firstAnn = (id: string): Record<string, Json> => {
  ...
  return (pages[0]!.annotations as Array<Record<string, Json>>)[0]!;
};

const gotoAnn = firstAnn('annotation-goto-over-dest');
const uriAnn = firstAnn('annotation-uri-over-dest');
const launchAnn = firstAnn('annotation-launch-file-url');
const gotoDest = gotoAnn.dest as Json[];
if (
  !Array.isArray(gotoDest) ||
  (gotoDest[1] as Record<string, Json>).name !== 'FitH' ||
  gotoDest[2] !== 10 ||
  Object.hasOwn(gotoAnn, 'url')
) {
  throw new Error('GoTo action must win over Dest with FitH/10 and no url');
}
if (uriAnn.url !== 'https://example.com/x' || Object.hasOwn(uriAnn, 'dest')) {
  throw new Error('URI action must expose url and suppress Dest');
}
if (launchAnn.url !== 'evil.exe' || Object.hasOwn(launchAnn, 'dest')) {
  throw new Error('Launch file must map to url evil.exe without dest');
}
...
console.error(
  `v3.0.14 annotation action precedence residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
