...
import {
  canonicalAnnotationDestResidualResult,
  type Json,
} from './v3014-annotation-dest-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-dest-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-dest-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-dest-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation dest residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation dest residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'fitFixture',
    fitFixture,
    baseline.fixtureSha256['v3014-annotation-dest-fit-v1.pdf']!,
  ],
  [
    'xyzFixture',
    xyzFixture,
    baseline.fixtureSha256['v3014-annotation-dest-xyz-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation dest residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation dest residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation dest residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation dest residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-dest-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationDestResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-annotation-dest-residual-relocated-'));
try {
  copyFileSync(fitFixture, join(relocatedDir, 'v3014-annotation-dest-fit-v1.pdf'));
  copyFileSync(xyzFixture, join(relocatedDir, 'v3014-annotation-dest-xyz-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation dest residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const fitCase = oracle.expectations['annotation-dest-fit-page-ref'] as Record<string, Json>;
const xyzCase = oracle.expectations['annotation-dest-xyz-page-ref'] as Record<string, Json>;
const fitAnns = (
  (
    (
      ((fitCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
...
const xyzAnns = (
  (
    (
      ((xyzCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
    ).annotations as Array<Record<string, Json>>
  )[0]!.annotations as Array<Record<string, Json>>
);
if (fitAnns.length !== 1 || fitAnns[0]!.subtype !== 'Link' || !Array.isArray(fitAnns[0]!.dest)) {
  throw new Error('fit case must expose one Link with dest array');
}
const fitDest = fitAnns[0]!.dest as Json[];
if (
  !fitDest[0] ||
  typeof fitDest[0] !== 'object' ||
  (fitDest[0] as Record<string, Json>).num !== 5 ||
  (fitDest[0] as Record<string, Json>).gen !== 0 ||
  !fitDest[1] ||
  typeof fitDest[1] !== 'object' ||
  (fitDest[1] as Record<string, Json>).name !== 'Fit'
) {
  throw new Error('fit dest must be pdf.js page-ref {num,gen} plus {name:Fit}');
}
if (xyzAnns.length !== 1 || !Array.isArray(xyzAnns[0]!.dest)) {
  throw new Error('xyz case must expose one Link with dest array');
}
const xyzDest = xyzAnns[0]!.dest as Json[];
if (
  (xyzDest[1] as Record<string, Json>).name !== 'XYZ' ||
  xyzDest[2] !== 72 ||
  xyzDest[3] !== 720 ||
  xyzDest[4] !== 0
) {
  throw new Error('xyz dest must keep name XYZ and coordinate numbers');
}
...
const report = {
  ...
  providerProof: {
    fitPageRefNameShape: true,
    xyzPageRefCoordinates: true,
    fullRectBoxes: true,
  },
...
console.error(
  `v3.0.14 annotation dest residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
