...
import {
  canonicalLineAnnotationResidualResult,
  type Json,
} from './v3014-line-annotation-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-line-annotation-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-line-annotation-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-line-annotation-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-line-annotation-residual-projection.ts');
const defaultFixture = join(fixtureDir, 'v3014-annotation-line-default-v1.pdf');
const lBboxFixture = join(fixtureDir, 'v3014-annotation-line-l-bbox-v1.pdf');
const border2Fixture = join(fixtureDir, 'v3014-annotation-line-border2-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 line annotation residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 line annotation residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['defaultFixture', defaultFixture, baseline.fixtureSha256['v3014-annotation-line-default-v1.pdf']!],
  ['lBboxFixture', lBboxFixture, baseline.fixtureSha256['v3014-annotation-line-l-bbox-v1.pdf']!],
  ['border2Fixture', border2Fixture, baseline.fixtureSha256['v3014-annotation-line-border2-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`line annotation residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`line annotation residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('line annotation residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust line annotation residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-line-annotation-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalLineAnnotationResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-text-annotation-relocated-'));
try {
  copyFileSync(defaultFixture, join(relocatedDir, 'v3014-annotation-line-default-v1.pdf'));
  copyFileSync(lBboxFixture, join(relocatedDir, 'v3014-annotation-line-l-bbox-v1.pdf'));
  copyFileSync(border2Fixture, join(relocatedDir, 'v3014-annotation-line-border2-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`line annotation residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const def = ((dataOf('line-default-border-expand').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const lBbox = ((
  dataOf('line-nonintersecting-rect-uses-l-bbox').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const border2 = ((dataOf('line-border-width-2-expand').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const defBox = (def[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const lBox = (lBbox[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const b2Box = (border2[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (defBox.left !== 71 || defBox.bottom !== 499 || defBox.right !== 201 || defBox.top !== 551) {
  throw new Error('default Line must expand public box by borderWidth=1');
}
if (lBox.left !== 7 || lBox.bottom !== 7 || lBox.right !== 103 || lBox.top !== 83) {
  throw new Error('non-intersecting Rect must adopt L bbox then expand by borderWidth');
}
if (b2Box.left !== 70 || b2Box.bottom !== 498 || b2Box.right !== 202 || b2Box.top !== 552) {
  throw new Error('BS W=2 Line must expand public box by 2');
}
if ((def[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line residual subtypes must remain Line');
}
...
const report = {
  ...
  providerProof: {
    defaultBorderExpand: true,
    nonIntersectingRectUsesLBbox: true,
    borderWidth2Expand: true,
  },
...
console.error(
  `v3.0.14 line annotation residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
