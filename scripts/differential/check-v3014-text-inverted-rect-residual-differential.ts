...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalTextInvertedRectResidualResult,
  type Json,
} from './v3014-text-inverted-rect-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-text-inverted-rect-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-text-inverted-rect-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-text-inverted-rect-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 text inverted-rect residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 text inverted-rect residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['invertedFixture', invertedFixture, baseline.fixtureSha256['v3014-annotation-text-inverted-v1.pdf']!],
  ['noApFixture', noApFixture, baseline.fixtureSha256['v3014-annotation-text-noap-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`text inverted-rect residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`text inverted-rect residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('text inverted-rect residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust text inverted-rect residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-text-inverted-rect-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalTextInvertedRectResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-text-inverted-rect-relocated-'));
try {
  copyFileSync(invertedFixture, join(relocatedDir, 'v3014-annotation-text-inverted-v1.pdf'));
  copyFileSync(noApFixture, join(relocatedDir, 'v3014-annotation-text-noap-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`text inverted-rect residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const inverted = ((dataOf('text-inverted-rect-icon-box').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const noAp = ((dataOf('text-no-appearance-icon-box').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const invertedBox = (inverted[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const noApBox = (noAp[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  invertedBox.left !== 100 ||
  invertedBox.bottom !== 678 ||
  invertedBox.right !== 122 ||
  invertedBox.top !== 700
) {
  throw new Error('Inverted Text no-appearance must normalize Rect then apply pdf.js 22x22 icon box');
}
if (
  noApBox.left !== 120 ||
  noApBox.bottom !== 678 ||
  ...
  throw new Error('Ordinary Text no-appearance residual must use pdf.js 22x22 icon box');
}
if ((inverted[0] as Record<string, Json>).contents !== 'Inverted') {
  throw new Error('inverted text contents mismatch');
}
if ((noAp[0] as Record<string, Json>).contents !== 'Sticky note') {
  throw new Error('no-appearance text contents mismatch');
}
...
const report = {
  ...
  providerProof: { invertedTextNoAppearanceIconBox: true, ordinaryTextNoAppearanceIconBox: true },
...
console.error(
  `v3.0.14 text inverted-rect residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
