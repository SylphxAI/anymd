...
import { canonicalFormCheckboxMultiExportResidualResult, type Json } from './v3014-form-checkbox-multi-export-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-multi-export-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-multi-export-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-multi-export-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-multi-export-residual-projection.ts');
const multiExportFooFixture = join(fixtureDir, 'v3014-form-checkbox-multi-export-foo-v1.pdf');
const multiExportAsBarFixture = join(fixtureDir, 'v3014-form-checkbox-multi-export-as-bar-v1.pdf');
const multiExportAsBazOffFixture = join(fixtureDir, 'v3014-form-checkbox-multi-export-as-baz-off-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-multi-export residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-multi-export residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['multiExportFooFixture', multiExportFooFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-foo-v1.pdf']!],
  ['multiExportAsBarFixture', multiExportAsBarFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-as-bar-v1.pdf']!],
  ['multiExportAsBazOffFixture', multiExportAsBazOffFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-as-baz-off-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-multi-export residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-multi-export residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-multi-export residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-multi-export residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-multi-export-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxMultiExportResidualResult(
...
try {
        copyFileSync(multiExportFooFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-foo-v1.pdf'));
  copyFileSync(multiExportAsBarFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-as-bar-v1.pdf'));
  copyFileSync(multiExportAsBazOffFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-as-baz-off-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-multi-export residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const fieldsOf = (id: string): Record<string, Json>[] => {
  const result = oracle.expectations[id] as Record<string, Json>;
  const data = ((result.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>;
  return data.form_fields as Record<string, Json>[];
};
const multiExportFoo = fieldsOf('form-checkbox-multi-export-foo')[0]!;
const multiExportAsBar = fieldsOf('form-checkbox-multi-export-as-bar')[0]!;
const multiExportAsBazOff = fieldsOf('form-checkbox-multi-export-as-baz-off')[0]!;
if (multiExportFoo.type !== 'checkbox' || multiExportFoo.value !== 'Foo' || multiExportFoo.default_value !== 'Off') {
  throw new Error('checkbox multi-export AP/N must keep admitted custom export Foo');
}
if (multiExportAsBar.type !== 'checkbox' || multiExportAsBar.value !== 'Bar' || multiExportAsBar.default_value !== 'Off') {
  throw new Error('checkbox multi-export AS=/Bar must switch to admitted export Bar');
}
if (multiExportAsBazOff.type !== 'checkbox' || multiExportAsBazOff.value !== 'Off' || multiExportAsBazOff.default_value !== 'Off') {
  throw new Error('checkbox multi-export AS=/Baz must force Off when not admitted');
}
...
const report = {
  ...
  providerProof: {
    checkboxMultiExportFoo: true,
    checkboxMultiExportAsBar: true,
    checkboxMultiExportAsBazOff: true,
  },
...
console.error(
  `v3.0.14 form checkbox-multi-export residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
