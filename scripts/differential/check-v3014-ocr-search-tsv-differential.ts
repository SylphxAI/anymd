...
import { canonicalOcrSearchMcpResult, type Json } from './v3014-ocr-search-tsv-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-ocr-search-tsv-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-ocr-search-tsv-oracle.json');
const runnerPath = join(scriptDir, 'v3014-ocr-search-tsv-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-ocr-search-tsv-projection.ts');
const sharedProjectionPath = join(scriptDir, 'v3014-ocr-search-projection.ts');
const providerPath = join(scriptDir, 'reference-ocr-search-tsv-provider.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 ocr-search tsv tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 ocr-search tsv tree mismatch');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-ocr-search-tsv-rust', version: '1' },
    });
    child.stdin.write(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-ocr-search-tsv-relocated-'));
try {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`ocr-search tsv comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const valid = oracle.expectations['ocr-search-tsv-valid-geometry'] as Record<string, Json>;
const malformed = oracle.expectations['ocr-search-tsv-malformed-soft'] as Record<string, Json>;
const validMatch = (((valid.results as Json[])[0] as Record<string, Json>).matches as Json[])[0] as Record<
  string,
  Json
>;
const malformedMatch = (((malformed.results as Json[])[0] as Record<string, Json>).matches as Json[])[0] as Record<
  string,
  Json
>;
if (!Object.hasOwn(validMatch, 'bounding_box')) {
  throw new Error('valid tsv search match must include geometry');
}
if (validMatch.bounding_box_level !== 'ocr_word') {
  throw new Error('valid tsv search match level must be ocr_word');
}
if (Object.hasOwn(malformedMatch, 'bounding_box') || Object.hasOwn(malformedMatch, 'bounding_box_level')) {
  throw new Error('malformed tsv soft fallback must omit geometry');
}
if (Object.hasOwn(malformedMatch, 'ocr_word_index')) {
  throw new Error('malformed tsv soft fallback must omit ocr_word_index');
}
...
const report = {
  ...
  providerProof: {
    ...
    validTsvWordGeometry: true,
    malformedTsvSoftFallback: true,
  },
...
console.error(
  `v3.0.14 ocr-search tsv differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
