...
import {
  canonicalOutlineGotorResidualResult,
  type Json,
} from './v3014-outline-gotor-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-outline-gotor-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-outline-gotor-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-outline-gotor-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-outline-gotor-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 outline GoToR residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 outline GoToR residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['stringFixture', stringFixture, baseline.fixtureSha256['v3014-outline-gotor-string-v1.pdf']!],
  ['tokenFixture', tokenFixture, baseline.fixtureSha256['v3014-outline-gotor-name-v1.pdf']!],
  ['gotoFixture', gotoFixture, baseline.fixtureSha256['v3014-outline-gotor-explicit-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`outline GoToR residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`outline GoToR residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('outline GoToR residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust outline GoToR residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-outline-gotor-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalOutlineGotorResidualResult(
...
try {
  copyFileSync(stringFixture, join(relocatedDir, 'v3014-outline-gotor-string-v1.pdf'));
  copyFileSync(tokenFixture, join(relocatedDir, 'v3014-outline-gotor-name-v1.pdf'));
  copyFileSync(gotoFixture, join(relocatedDir, 'v3014-outline-gotor-explicit-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`outline GoToR residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const stringCase = dataOf('outline-gotor-string').outline as Array<Record<string, Json>>;
const nameCase = dataOf('outline-gotor-name').outline as Array<Record<string, Json>>;
const explicitCase = dataOf('outline-gotor-explicit').outline as Array<Record<string, Json>>;
if (
  stringCase.length !== 1 ||
  stringCase[0]?.title !== 'Remote String' ||
  stringCase[0]?.url !== 'https://example.com/docs/other.pdf#Chapter1' ||
  stringCase[0]?.dest !== null
) {
  throw new Error('GoToR string D must project absolute url#Chapter1 with dest null');
}
if (
  nameCase.length !== 1 ||
  nameCase[0]?.title !== 'Remote Name' ||
  nameCase[0]?.url !== 'https://example.com/docs/other.pdf#SectionA' ||
  nameCase[0]?.dest !== null
) {
  throw new Error('GoToR name D must project absolute url#SectionA with dest null');
}
if (
  explicitCase.length !== 1 ||
  explicitCase[0]?.title !== 'Remote Explicit' ||
  explicitCase[0]?.url !== 'https://example.com/docs/remote.pdf#[0,{%22name%22:%22Fit%22}]' ||
  explicitCase[0]?.dest !== null
) {
  throw new Error('GoToR explicit D must project absolute url with encoded Fit dest JSON and dest null');
}
...
const report = {
  ...
  providerProof: {
    gotorStringDestUrl: true,
    gotorNameDestUrl: true,
    gotorExplicitDestUrl: true,
  },
...
console.error(
  `v3.0.14 outline GoToR residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
