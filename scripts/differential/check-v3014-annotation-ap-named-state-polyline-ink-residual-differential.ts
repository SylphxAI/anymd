...
import {
  canonicalAnnotationApNamedStatePolylineInkResidualResult,
  type Json,
} from './v3014-annotation-ap-named-state-polyline-ink-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-polyline-ink-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-ap-named-state-polyline-ink-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-ap-named-state-polyline-ink-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-ap-named-state-polyline-ink-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP named-state residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP named-state residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    ...
    lineApAsMissingFixture,
    baseline.fixtureSha256['v3014-annotation-ink-ap-as-missing-v1.pdf']!,
  ],
  [
    'lineApAsOnFixture',
    lineApAsOnFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-ap-as-on-v1.pdf']!,
  ],
  [
    'lineApAsInvalidFixture',
    lineApAsInvalidFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-ap-as-invalid-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP named-state residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP named-state residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP named-state residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP named-state residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-ap-named-state-polyline-ink-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationApNamedStatePolylineInkResidualResult(
...
try {
  copyFileSync(lineApAsMissingFixture, join(relocatedDir, 'v3014-annotation-ink-ap-as-missing-v1.pdf'));
  copyFileSync(lineApAsOnFixture, join(relocatedDir, 'v3014-annotation-polyline-ap-as-on-v1.pdf'));
  copyFileSync(lineApAsInvalidFixture, join(relocatedDir, 'v3014-annotation-polyline-ap-as-invalid-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP named-state residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const asOn = ((
  dataOf('polyline-ap-as-on-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asMissing = ((
  dataOf('ink-ap-as-missing-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const asInvalid = ((
  dataOf('polyline-ap-as-invalid-expands-geometry').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  ...
  throw new Error('PolyLine AP/N named-state with AS/On must keep raw Rect');
}
if (
  asMissingBox.left !== 26 ||
  asMissingBox.bottom !== 26 ||
  asMissingBox.right !== 104 ||
  asMissingBox.top !== 94
) {
  throw new Error('Ink AP/N named-state without AS must expand InkList');
}
if (
  asInvalidBox.left !== 6 ||
  asInvalidBox.bottom !== 6 ||
  asInvalidBox.right !== 104 ||
  asInvalidBox.top !== 84
) {
  throw new Error('PolyLine AP/N named-state with invalid AS must expand vertices');
}
if ((asOn[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline AP named-state residual subtype must remain PolyLine');
}
if ((asMissing[0] as Record<string, Json>).subtype !== 'Ink') {
  throw new Error('ink AP named-state residual subtype must remain Ink');
}
if ((asInvalid[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline AP named-state residual subtype must remain PolyLine');
}
...
const report = {
  ...
  providerProof: {
    polylineApAsOnKeepsRawRect: true,
    inkApAsMissingExpandsGeometry: true,
    polylineApAsInvalidExpandsGeometry: true,
  },
...
console.error(
  `v3.0.14 annotation AP named-state polyline/ink residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
