...
import { canonicalFormCheckboxExportEmptySingleResidualResult, type Json } from './v3014-form-checkbox-export-empty-single-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-export-empty-single-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-export-empty-single-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-export-empty-single-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-export-empty-single-residual-projection.ts');
const emptyApYesFixture = join(fixtureDir, 'v3014-form-checkbox-export-empty-ap-yes-v1.pdf');
const singleFooFixture = join(fixtureDir, 'v3014-form-checkbox-export-single-foo-v1.pdf');
const singleBarOffFixture = join(fixtureDir, 'v3014-form-checkbox-export-single-bar-off-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-export-empty-single residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-export-empty-single residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['emptyApYesFixture', emptyApYesFixture, baseline.fixtureSha256['v3014-form-checkbox-export-empty-ap-yes-v1.pdf']!],
  ['singleFooFixture', singleFooFixture, baseline.fixtureSha256['v3014-form-checkbox-export-single-foo-v1.pdf']!],
  ['singleBarOffFixture', singleBarOffFixture, baseline.fixtureSha256['v3014-form-checkbox-export-single-bar-off-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-export-empty-single residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-export-empty-single residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-export-empty-single residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-export-empty-single residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-export-empty-single-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxExportEmptySingleResidualResult(
...
try {
        copyFileSync(emptyApYesFixture, join(relocatedDir, 'v3014-form-checkbox-export-empty-ap-yes-v1.pdf'));
  copyFileSync(singleFooFixture, join(relocatedDir, 'v3014-form-checkbox-export-single-foo-v1.pdf'));
  copyFileSync(singleBarOffFixture, join(relocatedDir, 'v3014-form-checkbox-export-single-bar-off-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-export-empty-single residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const emptyApYes = fieldsOf('form-checkbox-export-empty-ap-yes')[0]!;
const singleFoo = fieldsOf('form-checkbox-export-single-foo')[0]!;
const singleBarOff = fieldsOf('form-checkbox-export-single-bar-off')[0]!;
if (emptyApYes.type !== 'checkbox' || emptyApYes.value !== 'Yes' || emptyApYes.default_value !== 'Off') {
  throw new Error('checkbox empty AP/N with AS=/Yes must admit Yes');
}
if (singleFoo.type !== 'checkbox' || singleFoo.value !== 'Foo' || singleFoo.default_value !== 'Off') {
  throw new Error('checkbox single non-Off AP key with AS=/Foo must admit Foo');
}
if (singleBarOff.type !== 'checkbox' || singleBarOff.value !== 'Off' || singleBarOff.default_value !== 'Off') {
  throw new Error('checkbox single non-Off AP key with AS=/Bar must force Off');
}
...
const report = {
  ...
  providerProof: {
    checkboxExportEmptyApYes: true,
    checkboxExportSingleFoo: true,
    checkboxExportSingleBarOff: true,
  },
...
console.error(
  `v3.0.14 form checkbox-export-empty-single residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
