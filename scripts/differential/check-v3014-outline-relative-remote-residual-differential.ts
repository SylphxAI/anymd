...
import {
  canonicalOutlineRelativeRemoteResidualResult,
  type Json,
} from './v3014-outline-relative-remote-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-outline-relative-remote-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-outline-relative-remote-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-outline-relative-remote-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-outline-relative-remote-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 outline relative-remote residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 outline relative-remote residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['gotorFixture', gotorFixture, baseline.fixtureSha256['v3014-outline-relative-gotor-v1.pdf']!],
  ['launchFixture', launchFixture, baseline.fixtureSha256['v3014-outline-relative-launch-v1.pdf']!],
  ['launchDestFixture', launchDestFixture, baseline.fixtureSha256['v3014-outline-relative-launch-dest-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`outline relative-remote residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`outline relative-remote residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('outline relative-remote residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust outline relative-remote residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-outline-relative-remote-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalOutlineRelativeRemoteResidualResult(
...
try {
  copyFileSync(gotorFixture, join(relocatedDir, 'v3014-outline-relative-gotor-v1.pdf'));
  copyFileSync(launchFixture, join(relocatedDir, 'v3014-outline-relative-launch-v1.pdf'));
  copyFileSync(launchDestFixture, join(relocatedDir, 'v3014-outline-relative-launch-dest-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`outline relative-remote residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const stringCase = dataOf('outline-relative-gotor').outline as Array<Record<string, Json>>;
const nameCase = dataOf('outline-relative-launch').outline as Array<Record<string, Json>>;
const explicitCase = dataOf('outline-relative-launch-dest').outline as Array<Record<string, Json>>;
if (
  stringCase.length !== 1 ||
  stringCase[0]?.title !== 'Relative GoToR' ||
  Object.hasOwn(stringCase[0] ?? {}, 'url') ||
  stringCase[0]?.dest !== null
) {
  throw new Error('relative GoToR must omit url and force dest null');
}
if (
  nameCase.length !== 1 ||
  nameCase[0]?.title !== 'Relative Launch' ||
  Object.hasOwn(nameCase[0] ?? {}, 'url') ||
  nameCase[0]?.dest !== null
) {
  throw new Error('relative Launch must omit url and force dest null');
}
if (
  explicitCase.length !== 1 ||
  explicitCase[0]?.title !== 'Relative Launch Dest' ||
  Object.hasOwn(explicitCase[0] ?? {}, 'url') ||
  explicitCase[0]?.dest !== null
) {
  throw new Error('relative Launch with sibling Dest must omit url and suppress Dest to null');
}
...
const report = {
  ...
  providerProof: {
    relativeGotorDropped: true,
    relativeLaunchDropped: true,
    relativeLaunchDestSuppressed: true,
  },
...
console.error(
  `v3.0.14 outline relative-remote residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
