...
const corpusPath = join(scriptDir, 'fixtures/v3014-encrypt-filter-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-encrypt-filter-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-encrypt-filter-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 encrypt filter residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 encrypt filter residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['unencryptedFixture', unencryptedFixture, baseline.fixtureSha256['v3014-permissions-none-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`encrypt filter residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`encrypt filter residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('encrypt filter residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust encrypt filter residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-encrypt-filter-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalEncryptFilterResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-encrypt-filter-relocated-'));
try {
  copyFileSync(encryptedFixture, join(relocatedDir, 'v3014-permissions-print-copy-fill-a11y-v1.pdf'));
  copyFileSync(unencryptedFixture, join(relocatedDir, 'v3014-permissions-none-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`encrypt filter residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const encrypted = dataOf('encrypt-filter-standard');
const unencrypted = dataOf('encrypt-filter-absent');
const encryptedInfo = encrypted.info as Record<string, Json>;
const unencryptedInfo = unencrypted.info as Record<string, Json>;
if (encryptedInfo.EncryptFilterName !== 'Standard') {
  throw new Error('encrypted residual must expose EncryptFilterName=Standard');
}
if (unencryptedInfo.EncryptFilterName !== null) {
  throw new Error('unencrypted residual must keep EncryptFilterName null');
}
if (encrypted.num_pages !== 1 || unencrypted.num_pages !== 1) {
  throw new Error('both residual cases must keep top-level num_pages=1');
}
...
const report = {
  ...
  providerProof: {
    encryptedStandardFilter: true,
    unencryptedNullFilter: true,
...
console.error(
  `v3.0.14 encrypt filter residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
