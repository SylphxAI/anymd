...
import {
  ...
  type Json,
} from './v3014-annotation-stamp-caret-file-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-stamp-caret-file-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-stamp-caret-file-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-stamp-caret-file-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-stamp-caret-file-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP stamp/caret/fileattachment residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP stamp/caret/fileattachment residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'stampFixture',
    stampFixture,
    baseline.fixtureSha256['v3014-annotation-stamp-basic-v1.pdf']!,
  ],
  [
    'caretFixture',
    caretFixture,
    baseline.fixtureSha256['v3014-annotation-caret-basic-v1.pdf']!,
  ],
  [
    'fileFixture',
    fileFixture,
    baseline.fixtureSha256['v3014-annotation-fileattachment-basic-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP stamp/caret/fileattachment residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP stamp/caret/fileattachment residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP stamp/caret/fileattachment residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP stamp/caret/fileattachment residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-stamp-caret-file-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationStampCaretFileResidualResult(
...
try {
  copyFileSync(stampFixture, join(relocatedDir, 'v3014-annotation-stamp-basic-v1.pdf'));
  copyFileSync(caretFixture, join(relocatedDir, 'v3014-annotation-caret-basic-v1.pdf'));
  copyFileSync(fileFixture, join(relocatedDir, 'v3014-annotation-fileattachment-basic-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP stamp/caret/fileattachment residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const stamp = ((
  dataOf('stamp-basic').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const caret = ((
  dataOf('caret-basic').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const fileAttachment = ((
  dataOf('fileattachment-basic').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const assertMarkup = (
  annotations: Json[],
  subtype: string,
  ...
  if (ann.subtype !== subtype || ann.contents !== contents || ann.title !== 'author' || ann.id !== '7R' || ann.page !== 1) {
    throw new Error(`${label}: unexpected annotation fields ${JSON.stringify(ann)}`);
  }
  const box = ann.bounding_box as Record<string, Json>;
  if (box.left !== 72 || box.bottom !== 600 || box.right !== 120 || box.top !== 640) {
    throw new Error(`${label}: expected raw Rect bounding_box, got ${JSON.stringify(box)}`);
  }
};
assertMarkup(stamp, 'Stamp', 'Stamp note', 'stamp');
assertMarkup(caret, 'Caret', 'Caret note', 'caret');
assertMarkup(fileAttachment, 'FileAttachment', 'FileAttachment note', 'fileattachment');
...
console.error(
  `v3.0.14 annotation AP stamp/caret/fileattachment residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
