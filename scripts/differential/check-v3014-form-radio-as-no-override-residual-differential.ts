...
import { canonicalFormRadioAsNoOverrideResidualResult, type Json } from './v3014-form-radio-as-no-override-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-radio-as-no-override-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-radio-as-no-override-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-radio-as-no-override-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-radio-as-no-override-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form radio-as-no-override residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form radio-as-no-override residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['radioNoOverrideFixture', radioNoOverrideFixture, baseline.fixtureSha256['v3014-form-radio-as-does-not-override-v-v1.pdf']!],
  ['radioInvalidKeepsFixture', radioInvalidKeepsFixture, baseline.fixtureSha256['v3014-form-radio-as-invalid-keeps-v-v1.pdf']!],
  ['checkboxRegressionFixture', checkboxRegressionFixture, baseline.fixtureSha256['v3014-form-radio-checkbox-as-regression-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form radio-as-no-override residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form radio-as-no-override residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form radio-as-no-override residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form radio-as-no-override residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-radio-as-no-override-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormRadioAsNoOverrideResidualResult(
...
try {
        copyFileSync(radioNoOverrideFixture, join(relocatedDir, 'v3014-form-radio-as-does-not-override-v-v1.pdf'));
  copyFileSync(radioInvalidKeepsFixture, join(relocatedDir, 'v3014-form-radio-as-invalid-keeps-v-v1.pdf'));
  copyFileSync(checkboxRegressionFixture, join(relocatedDir, 'v3014-form-radio-checkbox-as-regression-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form radio-as-no-override residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const radioNoOverride = fieldsOf('form-radio-as-does-not-override-v')[0]!;
const radioInvalidKeeps = fieldsOf('form-radio-as-invalid-keeps-v')[0]!;
const checkboxRegression = fieldsOf('form-radio-checkbox-as-regression')[0]!;
if (radioNoOverride.type !== 'radiobutton' || radioNoOverride.value !== 'Gold' || radioNoOverride.default_value !== 'Off') {
  throw new Error('radiobutton AS=/Silver must not override V=/Gold');
}
if (radioInvalidKeeps.type !== 'radiobutton' || radioInvalidKeeps.value !== 'Gold' || radioInvalidKeeps.default_value !== 'Off') {
  throw new Error('radiobutton invalid AS=/Maybe must keep V=/Gold (no export force-Off)');
}
if (checkboxRegression.type !== 'checkbox' || checkboxRegression.value !== 'Off' || checkboxRegression.default_value !== 'Off') {
  throw new Error('checkbox AS=/Off must still override V=/Yes');
}
...
const report = {
  ...
  providerProof: {
    radioAsDoesNotOverrideV: true,
    radioAsInvalidKeepsV: true,
    checkboxAsOverrideRegression: true,
  },
...
console.error(
  `v3.0.14 form radio-as-no-override residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
