...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalMetadataPresenceResidualResult,
  type Json,
} from './v3014-metadata-presence-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-metadata-presence-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-metadata-presence-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-metadata-presence-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 metadata presence residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 metadata presence residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['absentFixture', absentFixture, baseline.fixtureSha256['v3014-metadata-absent-v1.pdf']!],
  ['presentFixture', presentFixture, baseline.fixtureSha256['v3014-metadata-present-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`metadata presence residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`metadata presence residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('metadata presence residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust metadata presence residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-metadata-presence-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalMetadataPresenceResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-metadata-presence-relocated-'));
try {
  copyFileSync(absentFixture, join(relocatedDir, 'v3014-metadata-absent-v1.pdf'));
  copyFileSync(presentFixture, join(relocatedDir, 'v3014-metadata-present-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`metadata presence residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const absent = dataOf('metadata-absent');
const present = dataOf('metadata-present-empty-object');
if (absent.has_metadata !== false || Object.hasOwn(absent, 'metadata')) {
  throw new Error('absent catalog Metadata must omit metadata');
}
if (
  present.has_metadata !== true ||
  JSON.stringify(present.metadata) !== '{}' ||
  present.has_info !== true
) {
  throw new Error('present Metadata stream must emit empty metadata object with info');
}
...
const report = {
  ...
  providerProof: {
    absentOmitsMetadata: true,
    presentEmptyObject: true,
    infoStillPresent: true,
  },
...
console.error(
  `v3.0.14 metadata presence residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
