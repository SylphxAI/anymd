...
import {
  canonicalAnnotationResidualResult,
  type Json,
} from './v3014-annotation-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-residual-projection.ts');
const linkFreeTextFixture = join(fixtureDir, 'v3014-annotation-link-freetext-v1.pdf');
const textContentFixture = join(fixtureDir, 'v3014-annotation-text-content-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'linkFreeTextFixture',
    linkFreeTextFixture,
    baseline.fixtureSha256['v3014-annotation-link-freetext-v1.pdf']!,
  ],
  [
    'textContentFixture',
    textContentFixture,
    baseline.fixtureSha256['v3014-annotation-text-content-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-annotation-residual-relocated-'));
try {
  copyFileSync(linkFreeTextFixture, join(relocatedDir, 'v3014-annotation-link-freetext-v1.pdf'));
  copyFileSync(textContentFixture, join(relocatedDir, 'v3014-annotation-text-content-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const linkCase = oracle.expectations['annotation-link-freetext-full-boxes'] as Record<string, Json>;
const textCase = oracle.expectations['annotation-text-content-no-text-boxes'] as Record<
  string,
  Json
>;
const linkPages = (
  ((linkCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).annotations as Array<Record<string, Json>>;
const textPages = (
  ((textCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).annotations as Array<Record<string, Json>>;
const page1 = (linkPages[0]!.annotations as Array<Record<string, Json>>) ?? [];
const page2 = (linkPages[1]!.annotations as Array<Record<string, Json>>) ?? [];
if (page1.length !== 2 || page1[0]!.subtype !== 'FreeText' || !page1[0]!.bounding_box) {
  throw new Error('link/freetext case must keep FreeText full boxes on page 1');
}
if (page1[1]!.subtype !== 'Link' || page1[1]!.url !== 'https://example.com/note' || !page1[1]!.bounding_box) {
  throw new Error('link/freetext case must keep Link URI full boxes');
}
if (page2.length !== 1 || page2[0]!.subtype !== 'FreeText' || page2[0]!.contents !== 'Page two callout') {
  throw new Error('link/freetext case must preserve multi-page FreeText contents');
}
const textAnns = (textPages[0]!.annotations as Array<Record<string, Json>>) ?? [];
if (
  textAnns.length !== 2 ||
  textAnns[0]!.subtype !== 'Text' ||
  textAnns[0]!.contents !== 'Sticky note' ||
  textAnns[0]!.title !== 'Reviewer' ||
  Object.hasOwn(textAnns[0]!, 'bounding_box')
) {
  throw new Error('text content case must claim Text contents/title without bounding boxes');
}
if (textAnns[1]!.contents !== 'Second note' || Object.hasOwn(textAnns[1]!, 'bounding_box')) {
  throw new Error('second Text note must omit bounding boxes');
}
...
const report = {
  ...
  providerProof: {
    freeTextAndLinkFullBoxes: true,
    multiPageFreeText: true,
    textContentTitleWithoutBoxes: true,
  },
...
console.error(
  `v3.0.14 annotation residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
