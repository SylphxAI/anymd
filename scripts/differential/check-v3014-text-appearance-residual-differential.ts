...
import {
  canonicalTextAppearanceResidualResult,
  type Json,
} from './v3014-text-appearance-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-text-appearance-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-text-appearance-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-text-appearance-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 text appearance residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 text appearance residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['textApFixture', textApFixture, baseline.fixtureSha256['v3014-annotation-text-ap-v1.pdf']!],
  ['textEmptyApFixture', textEmptyApFixture, baseline.fixtureSha256['v3014-annotation-text-emptyap-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`text appearance residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`text appearance residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('text appearance residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust text appearance residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-text-appearance-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalTextAppearanceResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-text-appearance-relocated-'));
try {
  copyFileSync(textApFixture, join(relocatedDir, 'v3014-annotation-text-ap-v1.pdf'));
  copyFileSync(textEmptyApFixture, join(relocatedDir, 'v3014-annotation-text-emptyap-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`text appearance residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const withAp = ((dataOf('text-with-appearance-raw-rect').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const emptyAp = ((dataOf('text-empty-appearance-raw-rect').annotations as Json[])[0] as Record<string, Json>)
  .annotations as Json[];
const withBox = (withAp[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const emptyBox = (emptyAp[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  withBox.left !== 100 ||
  withBox.bottom !== 600 ||
  withBox.right !== 200 ||
  ...
  throw new Error('Text with appearance must keep raw Rect geometry');
}
if (
  emptyBox.left !== 50 ||
  emptyBox.bottom !== 600 ||
  ...
  throw new Error('Text with empty appearance stream must keep raw Rect geometry (not icon box)');
}
if ((withAp[0] as Record<string, Json>).contents !== 'With AP') {
  throw new Error('with-appearance contents mismatch');
}
if ((emptyAp[0] as Record<string, Json>).contents !== 'Empty AP') {
  throw new Error('empty-appearance contents mismatch');
}
...
const report = {
  ...
  providerProof: { textWithAppearanceKeepsRawRect: true, textEmptyAppearanceKeepsRawRect: true },
...
console.error(
  `v3.0.14 text appearance residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
