...
import {
  canonicalOutlineNamedDestResidualResult,
  type Json,
} from './v3014-outline-named-dest-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-outline-named-dest-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-outline-named-dest-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-outline-named-dest-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-outline-named-dest-residual-projection.ts');
const stringFixture = join(fixtureDir, 'v3014-outline-named-dest-string-v1.pdf');
const tokenFixture = join(fixtureDir, 'v3014-outline-named-dest-token-v1.pdf');
const gotoFixture = join(fixtureDir, 'v3014-outline-named-dest-goto-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 outline named-dest residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 outline named-dest residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['stringFixture', stringFixture, baseline.fixtureSha256['v3014-outline-named-dest-string-v1.pdf']!],
  ['tokenFixture', tokenFixture, baseline.fixtureSha256['v3014-outline-named-dest-token-v1.pdf']!],
  ['gotoFixture', gotoFixture, baseline.fixtureSha256['v3014-outline-named-dest-goto-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`outline named-dest residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`outline named-dest residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('outline named-dest residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust outline named-dest residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-outline-named-dest-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalOutlineNamedDestResidualResult(
...
try {
  copyFileSync(stringFixture, join(relocatedDir, 'v3014-outline-named-dest-string-v1.pdf'));
  copyFileSync(tokenFixture, join(relocatedDir, 'v3014-outline-named-dest-token-v1.pdf'));
  copyFileSync(gotoFixture, join(relocatedDir, 'v3014-outline-named-dest-goto-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`outline named-dest residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const stringCase = dataOf('outline-named-dest-string').outline as Array<Record<string, Json>>;
const tokenCase = dataOf('outline-named-dest-token').outline as Array<Record<string, Json>>;
const gotoCase = dataOf('outline-named-dest-goto').outline as Array<Record<string, Json>>;
if (
  stringCase.length !== 1 ||
  stringCase[0]?.title !== 'Named' ||
  stringCase[0]?.dest !== 'Chapter1'
) {
  throw new Error('Dest string must preserve named destination Chapter1 without resolving to array');
}
if (
  tokenCase.length !== 1 ||
  tokenCase[0]?.title !== 'NamedToken' ||
  tokenCase[0]?.dest !== 'Chapter1'
) {
  throw new Error('Dest name token must project as string Chapter1');
}
if (
  gotoCase.length !== 1 ||
  gotoCase[0]?.title !== 'ViaAction' ||
  gotoCase[0]?.dest !== 'Chapter1'
) {
  throw new Error('GoTo action D string must project as dest Chapter1');
}
...
const report = {
  ...
  providerProof: {
    namedDestStringPreserved: true,
    namedDestTokenPreserved: true,
    namedDestGotoActionPreserved: true,
  },
...
console.error(
  `v3.0.14 outline named-dest residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
