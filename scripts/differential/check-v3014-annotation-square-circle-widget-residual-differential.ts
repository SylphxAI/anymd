...
import {
  canonicalAnnotationSquareCircleWidgetResidualResult,
  type Json,
} from './v3014-annotation-square-circle-widget-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-square-circle-widget-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-square-circle-widget-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-square-circle-widget-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-square-circle-widget-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP square/circle/widget residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP square/circle/widget residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'squareFixture',
    squareFixture,
    baseline.fixtureSha256['v3014-annotation-square-inverted-v1.pdf']!,
  ],
  [
    'circleFixture',
    circleFixture,
    baseline.fixtureSha256['v3014-annotation-circle-inverted-v1.pdf']!,
  ],
  [
    'widgetFixture',
    widgetFixture,
    baseline.fixtureSha256['v3014-annotation-widget-field-t-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP square/circle/widget residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP square/circle/widget residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP square/circle/widget residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP square/circle/widget residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-square-circle-widget-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationSquareCircleWidgetResidualResult(
...
try {
  copyFileSync(squareFixture, join(relocatedDir, 'v3014-annotation-square-inverted-v1.pdf'));
  copyFileSync(circleFixture, join(relocatedDir, 'v3014-annotation-circle-inverted-v1.pdf'));
  copyFileSync(widgetFixture, join(relocatedDir, 'v3014-annotation-widget-field-t-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP square/circle/widget residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const square = ((
  dataOf('square-inverted').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const circle = ((
  dataOf('circle-inverted').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const widget = ((
  dataOf('widget-field-t').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const assertShape = (
  annotations: Json[],
  subtype: string,
  ...
  if (ann.subtype !== subtype || ann.contents !== contents || ann.id !== '7R' || ann.page !== 1) {
    throw new Error(`${label}: unexpected annotation fields ${JSON.stringify(ann)}`);
  }
  if (expectTitle === null) {
    if (Object.hasOwn(ann, 'title')) {
      throw new Error(`${label}: Widget must omit title (field /T is fieldName, not titleObj)`);
    }
  } else if (ann.title !== expectTitle) {
    throw new Error(`${label}: expected title ${expectTitle}, got ${JSON.stringify(ann.title)}`);
  }
...
assertShape(square, 'Square', 'Square note', { left: 100, bottom: 100, right: 200, top: 200 }, 'fieldOrAuthor', 'square');
assertShape(circle, 'Circle', 'Circle note', { left: 100, bottom: 100, right: 200, top: 200 }, 'fieldOrAuthor', 'circle');
assertShape(widget, 'Widget', 'Widget note', { left: 72, bottom: 600, right: 120, top: 620 }, null, 'widget');
...
console.error(
  `v3.0.14 annotation AP square/circle/widget residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
