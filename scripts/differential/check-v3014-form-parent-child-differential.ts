...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalFormParentChildResult,
  type Json,
} from './v3014-form-parent-child-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-parent-child-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-parent-child-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-parent-child-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form parent-child tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form parent-child tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['childFixture', childFixture, baseline.fixtureSha256['v3014-form-parent-child-v1.pdf']!],
  [
    'readonlyFixture',
    readonlyFixture,
    baseline.fixtureSha256['v3014-form-parent-readonly-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form parent-child ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form parent-child entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form parent-child bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form parent-child timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-parent-child-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormParentChildResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-form-parent-child-relocated-'));
try {
  copyFileSync(childFixture, join(relocatedDir, 'v3014-form-parent-child-v1.pdf'));
  copyFileSync(readonlyFixture, join(relocatedDir, 'v3014-form-parent-readonly-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form parent-child comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const childCase = oracle.expectations['form-parent-child-skip-directs-dv-value'] as Record<
  string,
  Json
>;
const readonlyCase = oracle.expectations['form-parent-readonly-editable-false'] as Record<
  string,
  Json
>;
const childFields = (
  ((childCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
const readonlyFields = (
  ((readonlyCase.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
if (childFields.length !== 2 || childFields[0]!.name !== 'parent' || Object.hasOwn(childFields[0]!, 'type')) {
  throw new Error('parent-child case must start with parent stub only');
}
if (childFields[1]!.name !== 'parent  .  child' || childFields[1]!.value !== 'fallback') {
  throw new Error('child must be dotted spaced name with DV-as-value fallback');
}
if (readonlyFields[1]!.editable !== false || readonlyFields[1]!.name !== 'owner.note') {
  throw new Error('readonly parent must yield non-editable dotted child owner.note');
}
...
const report = {
  ...
  providerProof: {
    skipDirectDicts: true,
    dottedChildNameWithDvValue: true,
    readonlyParentEditableFalse: true,
  },
...
console.error(
  `v3.0.14 form parent-child differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
