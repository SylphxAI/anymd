...
import { canonicalPopupZeroSizeResidualResult, type Json } from './v3014-popup-zero-size-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-popup-zero-size-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-popup-zero-size-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-popup-zero-size-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-popup-zero-size-residual-projection.ts');
const zeroFixture = join(fixtureDir, 'v3014-annotation-popup-zerosize-v1.pdf');
const nonzeroFixture = join(fixtureDir, 'v3014-annotation-popup-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') throw new Error('v3.0.14 popup zero-size residual tag moved');
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) throw new Error('v3.0.14 popup zero-size residual tree mismatch');
for (const [name, path, expected] of [['runner', runnerPath, baseline.runnerSha256], ['projection', projectionPath, baseline.projectionSha256], ['corpus', corpusPath, baseline.corpusSha256],   ['zeroFixture', zeroFixture, baseline.fixtureSha256['v3014-annotation-popup-zerosize-v1.pdf']!],
  ['nonzeroFixture', nonzeroFixture, baseline.fixtureSha256['v3014-annotation-popup-v1.pdf']!]] as const) {
  if (sha256(readFileSync(path)) !== expected) throw new Error(`popup zero-size residual ${name} digest drift`);
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) throw new Error(`popup zero-size residual entrypoint digest drift: ${path}`);
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-popup-zero-size-residual-rust', version: '1' } });
    ...
    return canonicalPopupZeroSizeResidualResult(await request(2, 'tools/call', { name: 'read_pdf', arguments: input }));
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-popup-zero-size-relocated-'));
try {
  copyFileSync(zeroFixture, join(relocatedDir, 'v3014-annotation-popup-zerosize-v1.pdf'));
  copyFileSync(nonzeroFixture, join(relocatedDir, 'v3014-annotation-popup-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) throw new Error(`popup zero-size residual comparator missed ${id}.${path.join('.')}`);
    leafMutationCount += 1;
  }
}
...
const zeroGroup = (dataOf('popup-zero-size-nulls-rect').annotations as Json[])[0] as Record<string, Json>;
const zeroAnns = zeroGroup.annotations as Json[];
const zeroPopup = zeroAnns.find((a) => (a as Record<string, Json>).subtype === 'Popup') as Record<string, Json>;
const zeroText = zeroAnns.find((a) => (a as Record<string, Json>).subtype === 'Text') as Record<string, Json>;
if (Object.hasOwn(zeroPopup, 'bounding_box')) throw new Error('zero-size popup must omit bounding_box');
if (zeroPopup.contents !== 'Parent note' || zeroPopup.title !== 'Author') throw new Error('zero-size popup must still inherit parent contents/title');
if (zeroText.contents !== 'Parent note') throw new Error('parent text contents mismatch');
const nonzeroGroup = (dataOf('popup-nonzero-keeps-rect').annotations as Json[])[0] as Record<string, Json>;
const nonzeroAnns = nonzeroGroup.annotations as Json[];
const nonzeroPopup = nonzeroAnns.find((a) => (a as Record<string, Json>).subtype === 'Popup') as Record<string, Json>;
if (!Object.hasOwn(nonzeroPopup, 'bounding_box')) throw new Error('nonzero popup must keep bounding_box');
const box = nonzeroPopup.bounding_box as Record<string, Json>;
if (box.left !== 130 || box.bottom !== 600 || box.right !== 250 || box.top !== 670) throw new Error('nonzero popup bounding_box mismatch');
...
const report = {
  ...
  providerProof: { popupZeroSizeNullsBoundingBox: true, popupNonzeroKeepsBoundingBox: true, popupZeroSizeStillInheritsParent: true },
...
console.error(`v3.0.14 popup zero-size residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`);
