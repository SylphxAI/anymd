...
import { canonicalFormCheckboxMultiExportManyResidualResult, type Json } from './v3014-form-checkbox-multi-export-many-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-multi-export-many-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-multi-export-many-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-multi-export-many-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-multi-export-many-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-multi-export-many residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-multi-export-many residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['manyCFixture', manyCFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-many-c-v1.pdf']!],
  ['manyAFixture', manyAFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-many-a-v1.pdf']!],
  ['manyZOffFixture', manyZOffFixture, baseline.fixtureSha256['v3014-form-checkbox-multi-export-many-z-off-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-multi-export-many residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-multi-export-many residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-multi-export-many residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-multi-export-many residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-multi-export-many-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxMultiExportManyResidualResult(
...
try {
        copyFileSync(manyCFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-many-c-v1.pdf'));
  copyFileSync(manyAFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-many-a-v1.pdf'));
  copyFileSync(manyZOffFixture, join(relocatedDir, 'v3014-form-checkbox-multi-export-many-z-off-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-multi-export-many residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const manyC = fieldsOf('form-checkbox-multi-export-many-c')[0]!;
const manyA = fieldsOf('form-checkbox-multi-export-many-a')[0]!;
const manyZOff = fieldsOf('form-checkbox-multi-export-many-z-off')[0]!;
if (manyC.type !== 'checkbox' || manyC.value !== 'C' || manyC.default_value !== 'Off') {
  throw new Error('checkbox multi-export many AS=/C must keep admitted export C');
}
if (manyA.type !== 'checkbox' || manyA.value !== 'A' || manyA.default_value !== 'Off') {
  throw new Error('checkbox multi-export many AS=/A must keep admitted export A');
}
if (manyZOff.type !== 'checkbox' || manyZOff.value !== 'Off' || manyZOff.default_value !== 'Off') {
  throw new Error('checkbox multi-export many AS=/Z must force Off when not admitted');
}
...
const report = {
  ...
  providerProof: {
    checkboxMultiExportManyC: true,
    checkboxMultiExportManyA: true,
    checkboxMultiExportManyZOff: true,
  },
...
console.error(
  `v3.0.14 form checkbox-multi-export-many residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
