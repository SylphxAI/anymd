...
import { canonicalRemoteNamedDestResidualResult, type Json } from './v3014-remote-named-dest-residual-projection.ts';
const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-remote-named-dest-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-remote-named-dest-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-remote-named-dest-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-remote-named-dest-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') throw new Error('v3.0.14 remote named dest residual tag moved');
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) throw new Error('v3.0.14 remote named dest residual tree mismatch');
for (const [name, path, expected] of [['runner', runnerPath, baseline.runnerSha256], ['projection', projectionPath, baseline.projectionSha256], ['corpus', corpusPath, baseline.corpusSha256],   ['launchStringFixture', launchStringFixture, baseline.fixtureSha256['v3014-annotation-gotor-named-string-v1.pdf']!],
  ['launchDictFixture', launchDictFixture, baseline.fixtureSha256['v3014-annotation-gotor-named-name-v1.pdf']!],
  ['gotorFixture', gotorFixture, baseline.fixtureSha256['v3014-annotation-gotor-v1.pdf']!]] as const) {
  if (sha256(readFileSync(path)) !== expected) throw new Error(`remote named dest residual ${name} digest drift`);
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) throw new Error(`remote named dest residual entrypoint digest drift: ${path}`);
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> => new Promise((resolve, reject) => {
    const timer = setTimeout(() => { reject(new Error(`Rust remote named dest residual timed out: ${stderr.slice(-2000)}`)); }, 120_000);
    pending.set(id, (value) => { clearTimeout(timer); resolve(value); });
    child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params })}\n`);
  });
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-remote-named-dest-residual-rust', version: '1' } });
    ...
    return canonicalRemoteNamedDestResidualResult(await request(2, 'tools/call', { name: 'read_pdf', arguments: input }));
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-remote-named-dest-relocated-'));
try {
  copyFileSync(launchStringFixture, join(relocatedDir, 'v3014-annotation-gotor-named-string-v1.pdf'));
  copyFileSync(launchDictFixture, join(relocatedDir, 'v3014-annotation-gotor-named-name-v1.pdf'));
  copyFileSync(gotorFixture, join(relocatedDir, 'v3014-annotation-gotor-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) throw new Error(`remote named dest residual comparator missed ${id}.${path.join('.')}`);
    leafMutationCount += 1;
  }
}
...
const ann = (id: string): Record<string, Json> => (((dataOf(id).annotations as Json[])[0] as Record<string, Json>).annotations as Json[])[0] as Record<string, Json>;
if (ann('gotor-named-string-dest').url !== 'other.pdf#Chapter1' || ann('gotor-named-string-dest').has_dest !== false) {
  throw new Error('gotor named string dest residual mismatch');
}
if (ann('gotor-named-name-dest').url !== 'other.pdf#Chapter1' || ann('gotor-named-name-dest').has_dest !== false) {
  throw new Error('gotor named name-object dest residual mismatch');
}
if (ann('gotor-explicit-dest-control').url !== 'other.pdf#[0,{"name":"Fit"}]' || ann('gotor-explicit-dest-control').has_dest !== false) {
  throw new Error('gotor explicit dest control residual mismatch');
}
...
const report = {
  ...
  providerProof: { gotorNamedStringDest: true, gotorNamedNameDest: true, gotorExplicitDestControl: true },
...
console.error(`v3.0.14 remote named dest residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`);
