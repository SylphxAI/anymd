...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalPageGeometryDepthClampResidualResult,
  type Json,
} from './v3014-page-geometry-depth-clamp-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-page-geometry-depth-clamp-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-page-geometry-depth-clamp-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-page-geometry-depth-clamp-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-page-geometry-depth-clamp-residual-projection.ts');
const deeperInheritFixture = join(fixtureDir, 'v3014-page-geometry-deeper-inherit-v1.pdf');
const nonRightAngleFixture = join(fixtureDir, 'v3014-page-geometry-non-right-angle-v1.pdf');
const cropIntersectFixture = join(fixtureDir, 'v3014-page-geometry-crop-intersect-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 page geometry depth-clamp residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 page geometry depth-clamp residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'deeperInheritFixture',
    deeperInheritFixture,
    baseline.fixtureSha256['v3014-page-geometry-deeper-inherit-v1.pdf']!,
  ],
  [
    'nonRightAngleFixture',
    nonRightAngleFixture,
    baseline.fixtureSha256['v3014-page-geometry-non-right-angle-v1.pdf']!,
  ],
  [
    'cropIntersectFixture',
    cropIntersectFixture,
    baseline.fixtureSha256['v3014-page-geometry-crop-intersect-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`page geometry depth-clamp residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`page geometry depth-clamp residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('page geometry depth-clamp residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust page geometry depth-clamp residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-page-geometry-depth-clamp-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalPageGeometryDepthClampResidualResult(
...
try {
  copyFileSync(deeperInheritFixture, join(relocatedDir, 'v3014-page-geometry-deeper-inherit-v1.pdf'));
  copyFileSync(nonRightAngleFixture, join(relocatedDir, 'v3014-page-geometry-non-right-angle-v1.pdf'));
  copyFileSync(cropIntersectFixture, join(relocatedDir, 'v3014-page-geometry-crop-intersect-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`page geometry depth-clamp residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const deeperInherit = geomOf('page-geometry-deeper-inherit');
const nonRightAngle = geomOf('page-geometry-non-right-angle');
const cropIntersect = geomOf('page-geometry-crop-intersect');
if (
  deeperInherit.width !== 200 ||
  deeperInherit.height !== 300 ||
  deeperInherit.rotation !== 0 ||
  deeperInherit.user_unit !== 1 ||
  (deeperInherit.view_box as Record<string, Json>).left !== 50 ||
  (deeperInherit.view_box as Record<string, Json>).bottom !== 60 ||
  (deeperInherit.view_box as Record<string, Json>).right !== 250 ||
  (deeperInherit.view_box as Record<string, Json>).top !== 360
) {
  throw new Error('deeper Pages MediaBox+CropBox inheritance must project view_box 50,60,250,360 at 200x300');
}
if (
  nonRightAngle.width !== 200 ||
  nonRightAngle.height !== 300 ||
  nonRightAngle.rotation !== 0 ||
  nonRightAngle.user_unit !== 1 ||
  (nonRightAngle.view_box as Record<string, Json>).left !== 0 ||
  (nonRightAngle.view_box as Record<string, Json>).bottom !== 0 ||
  (nonRightAngle.view_box as Record<string, Json>).right !== 200 ||
  (nonRightAngle.view_box as Record<string, Json>).top !== 300
) {
  throw new Error('non-right-angle Rotate 45 must clamp to rotation 0 with MediaBox dimensions 200x300');
}
if (
  cropIntersect.width !== 200 ||
  cropIntersect.height !== 200 ||
  cropIntersect.rotation !== 0 ||
  (cropIntersect.view_box as Record<string, Json>).left !== 0 ||
  (cropIntersect.view_box as Record<string, Json>).bottom !== 100 ||
  (cropIntersect.view_box as Record<string, Json>).right !== 200 ||
  (cropIntersect.view_box as Record<string, Json>).top !== 300
) {
  throw new Error('partial MediaBox/CropBox must project view_box 0,100,200,300 at 200x200');
}
...
const report = {
  ...
  providerProof: {
    deeperPagesInheritance: true,
    nonRightAngleClamp: true,
    partialCropIntersect: true,
  },
...
console.error(
  `v3.0.14 page geometry depth-clamp residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
