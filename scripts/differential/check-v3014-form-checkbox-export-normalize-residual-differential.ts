...
import { canonicalFormCheckboxExportNormalizeResidualResult, type Json } from './v3014-form-checkbox-export-normalize-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-export-normalize-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-export-normalize-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-export-normalize-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-export-normalize-residual-projection.ts');
const invalidExportFixture = join(fixtureDir, 'v3014-form-checkbox-as-invalid-export-off-v1.pdf');
const onlyOffYesFixture = join(fixtureDir, 'v3014-form-checkbox-as-only-off-yes-v1.pdf');
const offExportOkFixture = join(fixtureDir, 'v3014-form-checkbox-as-off-export-ok-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-export-normalize residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-export-normalize residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['invalidExportFixture', invalidExportFixture, baseline.fixtureSha256['v3014-form-checkbox-as-invalid-export-off-v1.pdf']!],
  ['onlyOffYesFixture', onlyOffYesFixture, baseline.fixtureSha256['v3014-form-checkbox-as-only-off-yes-v1.pdf']!],
  ['offExportOkFixture', offExportOkFixture, baseline.fixtureSha256['v3014-form-checkbox-as-off-export-ok-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-export-normalize residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-export-normalize residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-export-normalize residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-export-normalize residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-export-normalize-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxExportNormalizeResidualResult(
...
try {
        copyFileSync(invalidExportFixture, join(relocatedDir, 'v3014-form-checkbox-as-invalid-export-off-v1.pdf'));
  copyFileSync(onlyOffYesFixture, join(relocatedDir, 'v3014-form-checkbox-as-only-off-yes-v1.pdf'));
  copyFileSync(offExportOkFixture, join(relocatedDir, 'v3014-form-checkbox-as-off-export-ok-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-export-normalize residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const invalidExport = fieldsOf('form-checkbox-as-invalid-export-off')[0]!;
const onlyOffYes = fieldsOf('form-checkbox-as-only-off-yes')[0]!;
const offExportOk = fieldsOf('form-checkbox-as-off-export-ok')[0]!;
if (invalidExport.type !== 'checkbox' || invalidExport.value !== 'Off' || invalidExport.default_value !== 'Off') {
  throw new Error('checkbox AS=/Maybe must collapse to Off when exportValues are Off/Yes');
}
if (onlyOffYes.type !== 'checkbox' || onlyOffYes.value !== 'Yes' || onlyOffYes.default_value !== 'Off') {
  throw new Error('checkbox AP/N only-Off AP key must admit Yes from fieldValue and keep value Yes');
}
if (offExportOk.type !== 'checkbox' || offExportOk.value !== 'Off' || offExportOk.default_value !== 'Off') {
  throw new Error('checkbox AS=/Off valid export must keep Off');
}
...
const report = {
  ...
  providerProof: {
    checkboxInvalidExportOff: true,
    checkboxOnlyOffYes: true,
    checkboxOffExportOk: true,
  },
...
console.error(
  `v3.0.14 form checkbox-export-normalize residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
