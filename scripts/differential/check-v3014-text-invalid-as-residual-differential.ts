...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalTextInvalidAsResidualResult,
  type Json,
} from './v3014-text-invalid-as-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-text-invalid-as-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-text-invalid-as-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-text-invalid-as-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-text-invalid-as-residual-projection.ts');
const namedApFixture = join(fixtureDir, 'v3014-annotation-text-namedap-badas-v1.pdf');
const namedNoAsFixture = join(fixtureDir, 'v3014-annotation-text-namedap-as-nonstream-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 text named appearance residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 text named appearance residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['namedApFixture', namedApFixture, baseline.fixtureSha256['v3014-annotation-text-namedap-badas-v1.pdf']!],
  ['namedNoAsFixture', namedNoAsFixture, baseline.fixtureSha256['v3014-annotation-text-namedap-as-nonstream-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`text named appearance residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`text named appearance residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('text named appearance residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust text named appearance residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-text-invalid-as-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalTextInvalidAsResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-text-invalid-as-relocated-'));
try {
  copyFileSync(namedApFixture, join(relocatedDir, 'v3014-annotation-text-namedap-badas-v1.pdf'));
  copyFileSync(namedNoAsFixture, join(relocatedDir, 'v3014-annotation-text-namedap-as-nonstream-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`text named appearance residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const badAs = ((dataOf('named-appearance-invalid-as-icon-box').annotations as Json[])[0] as Record<
  string,
  Json
>).annotations as Json[];
const nonstream = ((
  dataOf('named-appearance-as-nonstream-icon-box').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const badAsBox = (badAs[0] as Record<string, Json>).bounding_box as Record<string, Json>;
const nonstreamBox = (nonstream[0] as Record<string, Json>).bounding_box as Record<string, Json>;
if (
  badAsBox.left !== 70 ||
  badAsBox.bottom !== 698 ||
  badAsBox.right !== 92 ||
  ...
  throw new Error('Invalid AS name must fall back to pdf.js 22x22 icon box');
}
if (
  nonstreamBox.left !== 60 ||
  nonstreamBox.bottom !== 688 ||
  nonstreamBox.right !== 82 ||
  nonstreamBox.top !== 710
) {
  throw new Error('AS non-stream selection must fall back to pdf.js 22x22 icon box');
}
if ((badAs[0] as Record<string, Json>).contents !== 'Bad AS') {
  throw new Error('invalid-as contents mismatch');
}
if ((nonstream[0] as Record<string, Json>).contents !== 'AS nonstream') {
  throw new Error('as-nonstream contents mismatch');
}
...
const report = {
  ...
  providerProof: { invalidAsNameUsesIconBox: true, asNonstreamUsesIconBox: true },
...
console.error(
  `v3.0.14 text invalid-as residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
