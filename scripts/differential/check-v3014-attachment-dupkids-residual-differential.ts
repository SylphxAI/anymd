...
import {
  canonicalAttachmentDupkidsResidualResult,
  type Json,
} from './v3014-attachment-dupkids-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-attachment-dupkids-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-attachment-dupkids-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-attachment-dupkids-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-attachment-dupkids-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 attachment dupkids residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 attachment dupkids residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['controlFixture', controlFixture, baseline.fixtureSha256['v3014-attachment-dupkids-control-v1.pdf']!],
  ['duplicateFixture', duplicateFixture, baseline.fixtureSha256['v3014-attachment-dupkids-duplicate-v1.pdf']!],
  ['cycleFixture', cycleFixture, baseline.fixtureSha256['v3014-attachment-dupkids-cycle-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`attachment dupkids residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`attachment dupkids residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('attachment dupkids residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust attachment dupkids residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-attachment-dupkids-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAttachmentDupkidsResidualResult(
...
try {
  copyFileSync(controlFixture, join(relocatedDir, 'v3014-attachment-dupkids-control-v1.pdf'));
  copyFileSync(duplicateFixture, join(relocatedDir, 'v3014-attachment-dupkids-duplicate-v1.pdf'));
  copyFileSync(cycleFixture, join(relocatedDir, 'v3014-attachment-dupkids-cycle-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`attachment dupkids residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const controlData = dataOf('attachment-dupkids-control') as Record<string, Json>;
const duplicateData = dataOf('attachment-dupkids-duplicate') as Record<string, Json>;
const cycleData = dataOf('attachment-dupkids-cycle') as Record<string, Json>;
const controlAtt = controlData.attachments as Array<Record<string, Json>>;
if (
  !Array.isArray(controlAtt) ||
  controlAtt.length !== 1 ||
  controlAtt[0]?.name !== 'child' ||
  controlAtt[0]?.filename !== 'ok.txt' ||
  controlAtt[0]?.size_bytes !== 1
) {
  throw new Error('control kids tree must expose one attachment child/ok.txt');
}
if (Object.hasOwn(duplicateData, 'attachments')) {
  throw new Error('duplicate kids must omit attachments surface');
}
if (Object.hasOwn(cycleData, 'attachments')) {
  throw new Error('cycle kids must omit attachments surface');
}
...
const report = {
  ...
  providerProof: {
    controlAttachmentPresent: true,
    duplicateKidsOmitted: true,
    cycleKidsOmitted: true,
  },
...
console.error(
  `v3.0.14 attachment dupkids residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
