...
const corpusPath = join(scriptDir, 'fixtures/v3014-form-text-multiline-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-text-multiline-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-text-multiline-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-text-multiline-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form text-multiline residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form text-multiline residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['lfFixture', lfFixture, baseline.fixtureSha256['v3014-form-text-multiline-lf-v1.pdf']!],
  ['crlfFixture', crlfFixture, baseline.fixtureSha256['v3014-form-text-multiline-crlf-v1.pdf']!],
  ['rawlfFixture', rawlfFixture, baseline.fixtureSha256['v3014-form-text-multiline-rawlf-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form text-multiline residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form text-multiline residual entrypoint digest drift: ${path}`);
  }
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form text-multiline residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-text-multiline-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormTextMultilineResidualResult(
...
try {
  copyFileSync(lfFixture, join(relocatedDir, 'v3014-form-text-multiline-lf-v1.pdf'));
  copyFileSync(crlfFixture, join(relocatedDir, 'v3014-form-text-multiline-crlf-v1.pdf'));
  copyFileSync(rawlfFixture, join(relocatedDir, 'v3014-form-text-multiline-rawlf-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form text-multiline residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const fieldValue = (caseId: string): string => {
  const expectation = oracle.expectations[caseId] as Record<string, Json>;
  const fields = (
    ((expectation.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
  ).form_fields as Array<Record<string, Json>>;
  if (fields.length !== 1 || fields[0]?.name !== 'notes' || typeof fields[0]?.value !== 'string') {
    throw new Error(`${caseId} must expose single notes text field`);
  }
  return fields[0]!.value as string;
};
if (fieldValue('form-text-multiline-lf') !== 'line1\nline2') {
  throw new Error('escaped LF multiline value must decode to line1\\nline2');
}
if (fieldValue('form-text-multiline-crlf') !== 'line1\r\nline2') {
  throw new Error('escaped CRLF multiline value must decode to line1\\r\\nline2');
}
if (fieldValue('form-text-multiline-rawlf') !== 'line1\nline2') {
  throw new Error('raw LF multiline value must decode to line1\\nline2');
}
...
const report = {
  ...
  providerProof: {
    escapedLfPreserved: true,
    escapedCrlfPreserved: true,
    rawLfPreserved: true,
  },
...
console.error(
  `v3.0.14 form text-multiline residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
