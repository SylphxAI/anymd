...
import { canonicalFormButtonDefaultOffResidualResult, type Json } from './v3014-form-button-default-off-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-button-default-off-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-button-default-off-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-button-default-off-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-button-default-off-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form button-default-off residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form button-default-off residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['checkboxApFixture', checkboxApFixture, baseline.fixtureSha256['v3014-form-checkbox-ap-default-off-v1.pdf']!],
  ['radioApFixture', radioApFixture, baseline.fixtureSha256['v3014-form-radio-ap-default-off-v1.pdf']!],
  ['checkboxNoApFixture', checkboxNoApFixture, baseline.fixtureSha256['v3014-form-checkbox-noap-default-null-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form button-default-off residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form button-default-off residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form button-default-off residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form button-default-off residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-button-default-off-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormButtonDefaultOffResidualResult(
...
try {
    copyFileSync(checkboxApFixture, join(relocatedDir, 'v3014-form-checkbox-ap-default-off-v1.pdf'));
  copyFileSync(radioApFixture, join(relocatedDir, 'v3014-form-radio-ap-default-off-v1.pdf'));
  copyFileSync(checkboxNoApFixture, join(relocatedDir, 'v3014-form-checkbox-noap-default-null-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form button-default-off residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const checkboxAp = fieldsOf('form-checkbox-ap-default-off')[0]!;
const radioAp = fieldsOf('form-radio-ap-default-off')[0]!;
const checkboxNoAp = fieldsOf('form-checkbox-noap-default-null')[0]!;
if (checkboxAp.type !== 'checkbox' || checkboxAp.value !== 'Yes' || checkboxAp.default_value !== 'Off') {
  throw new Error('checkbox with AP/N named states and missing DV must default_value Off');
}
if (radioAp.type !== 'radiobutton' || radioAp.value !== 'Gold' || radioAp.default_value !== 'Off') {
  throw new Error('radiobutton with AP/N named states and missing DV must default_value Off');
}
if (checkboxNoAp.type !== 'checkbox' || checkboxNoAp.value !== 'Yes' || checkboxNoAp.default_value !== null) {
  throw new Error('checkbox without AP and missing DV must keep default_value null');
}
...
const report = {
  ...
  providerProof: {
    checkboxApDefaultOff: true,
    radioApDefaultOff: true,
    checkboxNoApDefaultNull: true,
  },
...
console.error(
  `v3.0.14 form button-default-off residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
