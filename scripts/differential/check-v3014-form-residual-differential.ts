...
import { canonicalFormResidualResult, type Json } from './v3014-form-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['residualFixture', residualFixture, baseline.fixtureSha256['v3014-form-residual-v1.pdf']!],
  ['coercionFixture', coercionFixture, baseline.fixtureSha256['v3014-form-coercion-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormResidualResult(
...
try {
  copyFileSync(residualFixture, join(relocatedDir, 'v3014-form-residual-v1.pdf'));
  copyFileSync(coercionFixture, join(relocatedDir, 'v3014-form-coercion-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const kinds = oracle.expectations['form-radio-pushbutton-signature-kinds'] as Record<string, Json>;
const coercion = oracle.expectations['form-value-coercion-residual'] as Record<string, Json>;
const kindFields = (
  ((kinds.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
const coercionFields = (
  ((coercion.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
if (kindFields.map((f) => f.type).join(',') !== 'radiobutton,button,signature') {
  throw new Error('kinds case must expose radiobutton, button, signature in order');
}
const signature = kindFields[2]!;
if (Object.hasOwn(signature, 'bounding_box') || Object.hasOwn(signature, 'editable')) {
  throw new Error('signature must omit bounding_box and editable');
}
if (coercionFields.find((f) => f.name === 'numeric')?.value !== '') {
  throw new Error('numeric text with non-string V must coerce to empty string');
}
if (coercionFields.find((f) => f.name === 'button')?.type !== 'checkbox') {
  throw new Error('bool button without radio/push flags must be checkbox');
}
if (coercionFields.find((f) => f.name === 'choice')?.value !== 'first') {
  throw new Error('choice array V must take first string entry');
}
...
const report = {
  ...
  providerProof: {
    radiobuttonPushbuttonSignatureKinds: true,
    signatureOmitsBoxAndEditable: true,
    valueCoercionResidual: true,
  },
...
console.error(
  `v3.0.14 form residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
