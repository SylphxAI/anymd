...
import {
  canonicalAnnotationAppearanceBboxResidualResult,
  type Json,
} from './v3014-annotation-appearance-bbox-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-appearance-bbox-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-appearance-bbox-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-appearance-bbox-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-appearance-bbox-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation appearance bbox residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation appearance bbox residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'polylineAppearanceBboxFixture',
    polylineAppearanceBboxFixture,
    baseline.fixtureSha256['v3014-annotation-polyline-ap-bbox-v1.pdf']!,
  ],
  [
    'lineAppearanceBboxFixture',
    lineAppearanceBboxFixture,
    baseline.fixtureSha256['v3014-annotation-line-ap-bbox-v1.pdf']!,
  ],
  [
    'inkAppearanceBboxFixture',
    inkAppearanceBboxFixture,
    baseline.fixtureSha256['v3014-annotation-ink-ap-bbox-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation appearance bbox residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation appearance bbox residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation appearance bbox residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation appearance bbox residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-appearance-bbox-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationAppearanceBboxResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-annotation-appearance-bbox-relocated-'));
try {
  copyFileSync(polylineAppearanceBboxFixture, join(relocatedDir, 'v3014-annotation-polyline-ap-bbox-v1.pdf'));
  copyFileSync(lineAppearanceBboxFixture, join(relocatedDir, 'v3014-annotation-line-ap-bbox-v1.pdf'));
  copyFileSync(inkAppearanceBboxFixture, join(relocatedDir, 'v3014-annotation-ink-ap-bbox-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation appearance bbox residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const polyline = ((
  dataOf('polyline-appearance-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const line = ((
  dataOf('line-appearance-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const ink = ((
  dataOf('ink-appearance-keeps-raw-rect').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
...
if (
  polylineBox.left !== 200 ||
  polylineBox.bottom !== 200 ||
  polylineBox.right !== 300 ||
  polylineBox.top !== 300
) {
  throw new Error('PolyLine with AP/N must keep raw Rect and skip vertices expansion');
}
if (
  lineBox.left !== 200 ||
  lineBox.bottom !== 200 ||
  ...
  throw new Error('Line with AP/N must keep raw Rect and skip L expansion');
}
if (
  inkBox.left !== 200 ||
  inkBox.bottom !== 200 ||
  ...
  throw new Error('Ink with AP/N must keep raw Rect and skip InkList expansion');
}
if ((polyline[0] as Record<string, Json>).subtype !== 'PolyLine') {
  throw new Error('polyline annotation appearance bbox residual subtype must remain PolyLine');
}
if ((line[0] as Record<string, Json>).subtype !== 'Line') {
  throw new Error('line annotation appearance bbox residual subtype must remain Line');
}
if ((ink[0] as Record<string, Json>).subtype !== 'Ink') {
  throw new Error('ink annotation appearance bbox residual subtype must remain Ink');
}
...
const report = {
  ...
  providerProof: {
    polylineAppearanceKeepsRawRect: true,
    lineAppearanceKeepsRawRect: true,
    inkAppearanceKeepsRawRect: true,
  },
...
console.error(
  `v3.0.14 annotation appearance bbox residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
