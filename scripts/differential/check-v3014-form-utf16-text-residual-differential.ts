...
import { canonicalFormUtf16TextResidualResult, type Json } from './v3014-form-utf16-text-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-utf16-text-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-utf16-text-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-utf16-text-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form utf16-text residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form utf16-text residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['utf16Fixture', utf16Fixture, baseline.fixtureSha256['v3014-form-utf16-odd-v1.pdf']!],
  ['utf8Fixture', utf8Fixture, baseline.fixtureSha256['v3014-form-utf8-bom-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form utf16-text residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form utf16-text residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form utf16-text residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form utf16-text residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-utf16-text-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormUtf16TextResidualResult(
...
try {
  copyFileSync(utf16Fixture, join(relocatedDir, 'v3014-form-utf16-odd-v1.pdf'));
  copyFileSync(utf8Fixture, join(relocatedDir, 'v3014-form-utf8-bom-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form utf16-text residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const utf16 = oracle.expectations['form-utf16-odd-and-plain'] as Record<string, Json>;
const utf8 = oracle.expectations['form-utf8-bom'] as Record<string, Json>;
const utf16Fields = (
  ((utf16.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
const utf8Fields = (
  ((utf8.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).form_fields as Array<Record<string, Json>>;
if (utf16Fields.map((f) => f.name).join(',') !== 'valid,odd,plain') {
  throw new Error('utf16 case must expose valid,odd,plain in order');
}
if (utf16Fields.find((f) => f.name === 'valid')?.value !== 'Ada') {
  throw new Error('valid UTF-16BE BOM must decode to Ada');
}
if (
  utf16Fields.find((f) => f.name === 'odd')?.value !== 'Bo' ||
  utf16Fields.find((f) => f.name === 'odd')?.default_value !== 'Bo'
) {
  throw new Error('odd-length UTF-16BE must drop trailing byte to Bo for V and DV');
}
if (utf16Fields.find((f) => f.name === 'plain')?.value !== 'Alice') {
  throw new Error('PDFDocEncoding plain control must remain Alice');
}
if (utf8Fields.length !== 1 || utf8Fields[0]?.name !== 'utf8' || utf8Fields[0]?.value !== 'Hi') {
  throw new Error('UTF-8 BOM form value must decode to Hi');
}
...
const report = {
  ...
  providerProof: {
    utf16BeValidBom: true,
    utf16BeOddLengthDropsTrailingByte: true,
    utf8BomAndPdfDocEncodingControls: true,
  },
...
console.error(
  `v3.0.14 form utf16-text residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
