...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalAnnotationLinkAaActionResidualResult,
  type Json,
} from './v3014-annotation-link-aa-action-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-link-aa-action-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-link-aa-action-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-link-aa-action-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-link-aa-action-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation AP link-aa-action residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation AP link-aa-action residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'aaUFixture',
    aaUFixture,
    baseline.fixtureSha256['v3014-annotation-link-aa-u-uri-v1.pdf']!,
  ],
  [
    'aaDFixture',
    aaDFixture,
    baseline.fixtureSha256['v3014-annotation-link-aa-d-goto-v1.pdf']!,
  ],
  [
    'aWinsFixture',
    aWinsFixture,
    baseline.fixtureSha256['v3014-annotation-link-a-wins-over-aa-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation AP link-aa-action residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation AP link-aa-action residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation AP link-aa-action residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation AP link-aa-action residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-link-aa-action-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationLinkAaActionResidualResult(
...
try {
  copyFileSync(aaUFixture, join(relocatedDir, 'v3014-annotation-link-aa-u-uri-v1.pdf'));
  copyFileSync(aaDFixture, join(relocatedDir, 'v3014-annotation-link-aa-d-goto-v1.pdf'));
  copyFileSync(aWinsFixture, join(relocatedDir, 'v3014-annotation-link-a-wins-over-aa-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation AP link-aa-action residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const aaU = ((
  dataOf('link-aa-u-uri').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const aaD = ((
  dataOf('link-aa-d-goto').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const aWins = ((
  dataOf('link-a-wins-over-aa').annotations as Json[]
)[0] as Record<string, Json>).annotations as Json[];
const assertAnn = (
  annotations: Json[],
  ...
  for (const [key, value] of Object.entries(expect)) {
    if (ann[key] !== value) {
      throw new Error(`${label}: expected ${key}=${JSON.stringify(value)}, got ${JSON.stringify(ann[key])} in ${JSON.stringify(ann)}`);
    }
  }
};
assertAnn(aaU, { subtype: 'Link', url: 'https://example.com/aa', id: '7R', page: 1 }, 'AA/U URI');
assertAnn(aaD, { subtype: 'Link', dest: 'SectionAA', id: '7R', page: 1 }, 'AA/D GoTo');
assertAnn(aWins, { subtype: 'Link', url: 'https://example.com/a', id: '7R', page: 1 }, 'A wins over AA');
if (Object.hasOwn(aWins[0] as Record<string, Json>, 'dest')) {
  throw new Error('A/URI must suppress dest');
}
if (Object.hasOwn(aaD[0] as Record<string, Json>, 'url')) {
  throw new Error('AA/D GoTo must not project url');
}
...
console.error(
  `v3.0.14 annotation AP link-aa-action residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
