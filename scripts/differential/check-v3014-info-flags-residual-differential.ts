...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  canonicalInfoFlagsResidualResult,
  type Json,
} from './v3014-info-flags-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-info-flags-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-info-flags-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-info-flags-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-info-flags-residual-projection.ts');
const acroformFixture = join(fixtureDir, 'v3014-info-flags-acroform-v1.pdf');
const plainFixture = join(fixtureDir, 'v3014-info-flags-plain-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 info flags residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 info flags residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['acroformFixture', acroformFixture, baseline.fixtureSha256['v3014-info-flags-acroform-v1.pdf']!],
  ['plainFixture', plainFixture, baseline.fixtureSha256['v3014-info-flags-plain-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`info flags residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`info flags residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('info flags residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust info flags residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-info-flags-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalInfoFlagsResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-info-flags-relocated-'));
try {
  copyFileSync(acroformFixture, join(relocatedDir, 'v3014-info-flags-acroform-v1.pdf'));
  copyFileSync(plainFixture, join(relocatedDir, 'v3014-info-flags-plain-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`info flags residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const infoOf = (id: string): Record<string, Json> => {
  ...
  ).info as Record<string, Json>;
};
const acro = infoOf('info-flags-acroform-lang');
const plain = infoOf('info-flags-plain');
if (
  acro.IsAcroFormPresent !== true ||
  acro.Language !== 'en-US' ||
  acro.EncryptFilterName !== null ||
  acro.IsLinearized !== false ||
  acro.IsXFAPresent !== false ||
  acro.IsCollectionPresent !== false ||
  acro.IsSignaturesPresent !== false
) {
  throw new Error('acroform case must expose pdf.js info flags with Lang=en-US and AcroForm true');
}
if (
  plain.IsAcroFormPresent !== false ||
  plain.Language !== null ||
  plain.EncryptFilterName !== null ||
  plain.IsLinearized !== false
) {
  throw new Error('plain case must expose false/null pdf.js info flags');
}
...
const report = {
  ...
  providerProof: {
    acroformLangFlags: true,
    plainNullFlags: true,
    documentInfoFields: true,
  },
...
console.error(
  `v3.0.14 info flags residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
