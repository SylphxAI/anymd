...
import {
  canonicalAnnotationContentsMultilineResidualResult,
  type Json,
} from './v3014-annotation-contents-multiline-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-annotation-contents-multiline-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-annotation-contents-multiline-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-annotation-contents-multiline-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-annotation-contents-multiline-residual-projection.ts');
const lfFixture = join(fixtureDir, 'v3014-annotation-contents-multiline-freetext-lf-v1.pdf');
const crlfFixture = join(fixtureDir, 'v3014-annotation-contents-multiline-freetext-crlf-v1.pdf');
const textLfFixture = join(fixtureDir, 'v3014-annotation-contents-multiline-text-lf-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 annotation contents-multiline residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 annotation contents-multiline residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  [
    'lfFixture',
    lfFixture,
    baseline.fixtureSha256['v3014-annotation-contents-multiline-freetext-lf-v1.pdf']!,
  ],
  [
    'crlfFixture',
    crlfFixture,
    baseline.fixtureSha256['v3014-annotation-contents-multiline-freetext-crlf-v1.pdf']!,
  ],
  [
    'textLfFixture',
    textLfFixture,
    baseline.fixtureSha256['v3014-annotation-contents-multiline-text-lf-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`annotation contents-multiline residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`annotation contents-multiline residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('annotation contents-multiline residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (
    ...
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust annotation contents-multiline residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-annotation-contents-multiline-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAnnotationContentsMultilineResidualResult(
...
try {
  copyFileSync(lfFixture, join(relocatedDir, 'v3014-annotation-contents-multiline-freetext-lf-v1.pdf'));
  copyFileSync(crlfFixture, join(relocatedDir, 'v3014-annotation-contents-multiline-freetext-crlf-v1.pdf'));
  copyFileSync(textLfFixture, join(relocatedDir, 'v3014-annotation-contents-multiline-text-lf-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`annotation contents-multiline residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const annsOf = (id: string): Json[] =>
  ((dataOf(id).annotations as Json[])[0] as Record<string, Json>).annotations as Json[];
const assertContents = (
  id: string,
  subtype: string,
  contents: string,
  box: { left: number; bottom: number; right: number; top: number }
) => {
  const annotations = annsOf(id);
  if (annotations.length !== 1) {
    throw new Error(`${id}: expected 1 annotation, got ${String(annotations.length)}`);
  }
  ...
  if (
    ...
    throw new Error(`${id}: unexpected annotation fields ${JSON.stringify(ann)}`);
  }
  const b = ann.bounding_box as Record<string, Json>;
  if (b.left !== box.left || b.bottom !== box.bottom || b.right !== box.right || b.top !== box.top) {
    throw new Error(`${id}: expected bbox ${JSON.stringify(box)}, got ${JSON.stringify(b)}`);
  }
};
assertContents(
  'annotation-contents-multiline-freetext-lf',
  'FreeText',
  'line1\nline2',
  { left: 72, bottom: 600, right: 200, top: 650 }
);
assertContents(
  'annotation-contents-multiline-freetext-crlf',
  'FreeText',
  'line1\r\nline2',
  { left: 72, bottom: 600, right: 200, top: 650 }
);
assertContents(
  'annotation-contents-multiline-text-lf',
  'Text',
  'line1\nline2',
  { left: 72, bottom: 628, right: 94, top: 650 }
);
...
const report = {
  ...
  providerProof: {
    freeTextEscapedLfPreserved: true,
    freeTextEscapedCrlfPreserved: true,
    textStickyEscapedLfPreserved: true,
  },
...
console.error(
  `v3.0.14 annotation contents-multiline residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
