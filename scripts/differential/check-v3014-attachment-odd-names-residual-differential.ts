...
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
  ...
  type Json,
} from './v3014-attachment-odd-names-residual-projection.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-attachment-odd-names-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-attachment-odd-names-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-attachment-odd-names-residual-baseline-runner.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 attachment odd-names residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 attachment odd-names residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['orphanFixture', orphanFixture, baseline.fixtureSha256['v3014-attachment-odd-names-v1.pdf']!],
  [
    'pairFixture',
    pairFixture,
    baseline.fixtureSha256['v3014-attachment-odd-names-pair-v1.pdf']!,
  ],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`attachment odd-names residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`attachment odd-names residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('attachment odd-names residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust attachment odd-names residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-attachment-odd-names-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalAttachmentOddNamesResidualResult(
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-attachment-residual-relocated-'));
try {
  copyFileSync(orphanFixture, join(relocatedDir, 'v3014-attachment-odd-names-v1.pdf'));
  copyFileSync(pairFixture, join(relocatedDir, 'v3014-attachment-odd-names-pair-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`attachment odd-names residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}

const orphanOnly = oracle.expectations['attachment-odd-names-orphan-only'] as Record<string, Json>;
const pairPlus = oracle.expectations['attachment-odd-names-pair-plus-orphan'] as Record<
  string,
  Json
>;
const orphanAtt = (
  ((orphanOnly.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).attachments as Array<Record<string, Json>>;
const pairAtt = (
  ((pairPlus.results as Json[])[0] as Record<string, Json>).data as Record<string, Json>
).attachments as Array<Record<string, Json>>;
if (
  orphanAtt.length !== 1 ||
  orphanAtt[0]!.name !== 'orphan' ||
  orphanAtt[0]!.filename !== 'unnamed' ||
  Object.hasOwn(orphanAtt[0]!, 'size_bytes')
) {
  throw new Error('orphan-only case must expose orphan/unnamed without size_bytes');
}
if (pairAtt.length !== 2) throw new Error('pair+orphan case must expose two attachments');
if (
  pairAtt[0]!.name !== 'child' ||
  pairAtt[0]!.filename !== 'a.txt' ||
  pairAtt[0]!.size_bytes !== 1
) {
  throw new Error('pair+orphan must keep complete child/a.txt pair first');
}
if (
  pairAtt[1]!.name !== 'orphan' ||
  pairAtt[1]!.filename !== 'unnamed' ||
  Object.hasOwn(pairAtt[1]!, 'size_bytes')
) {
  throw new Error('pair+orphan must materialize trailing orphan without size_bytes');
}
...
const report = {
  ...
  providerProof: {
    orphanOnlyOddNames: true,
    pairPlusOrphanKeepsCompletePair: true,
    trailingOrphanUnnamedNoSize: true,
  },
...
console.error(
  `v3.0.14 attachment odd-names residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
