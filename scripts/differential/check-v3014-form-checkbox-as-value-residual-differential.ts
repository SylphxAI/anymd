...
import { canonicalFormCheckboxAsValueResidualResult, type Json } from './v3014-form-checkbox-as-value-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';

const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-form-checkbox-as-value-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-form-checkbox-as-value-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-form-checkbox-as-value-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-form-checkbox-as-value-residual-projection.ts');
const asOffFixture = join(fixtureDir, 'v3014-form-checkbox-as-overrides-v-off-v1.pdf');
const asYesFixture = join(fixtureDir, 'v3014-form-checkbox-as-overrides-v-yes-v1.pdf');
const noApFixture = join(fixtureDir, 'v3014-form-checkbox-as-noap-keeps-v-v1.pdf');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') {
  throw new Error('v3.0.14 form checkbox-as-value residual tag moved');
}
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) {
  throw new Error('v3.0.14 form checkbox-as-value residual tree mismatch');
}
for (const [name, path, expected] of [
  ...
  ['asOffFixture', asOffFixture, baseline.fixtureSha256['v3014-form-checkbox-as-overrides-v-off-v1.pdf']!],
  ['asYesFixture', asYesFixture, baseline.fixtureSha256['v3014-form-checkbox-as-overrides-v-yes-v1.pdf']!],
  ['noApFixture', noApFixture, baseline.fixtureSha256['v3014-form-checkbox-as-noap-keeps-v-v1.pdf']!],
] as const) {
  if (sha256(readFileSync(path)) !== expected) {
    throw new Error(`form checkbox-as-value residual ${name} digest drift`);
  }
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) {
    throw new Error(`form checkbox-as-value residual entrypoint digest drift: ${path}`);
  }
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) {
  throw new Error('form checkbox-as-value residual bun.lock digest drift');
}
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> =>
    new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error(`Rust form checkbox-as-value residual timed out: ${stderr.slice(-2000)}`));
      }, 120_000);
  ...
  try {
    await request(1, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'v3014-form-checkbox-as-value-residual-rust', version: '1' },
    });
    child.stdin.write(
    ...
    return canonicalFormCheckboxAsValueResidualResult(
...
try {
        copyFileSync(asOffFixture, join(relocatedDir, 'v3014-form-checkbox-as-overrides-v-off-v1.pdf'));
  copyFileSync(asYesFixture, join(relocatedDir, 'v3014-form-checkbox-as-overrides-v-yes-v1.pdf'));
  copyFileSync(noApFixture, join(relocatedDir, 'v3014-form-checkbox-as-noap-keeps-v-v1.pdf'));
  for (const entry of corpus.cases) {
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) {
      throw new Error(`form checkbox-as-value residual comparator missed ${id}.${path.join('.')}`);
    }
    leafMutationCount += 1;
  }
}
...
const asOff = fieldsOf('form-checkbox-as-overrides-v-off')[0]!;
const asYes = fieldsOf('form-checkbox-as-overrides-v-yes')[0]!;
const noAp = fieldsOf('form-checkbox-as-noap-keeps-v')[0]!;
if (asOff.type !== 'checkbox' || asOff.value !== 'Off' || asOff.default_value !== 'Off') {
  throw new Error('checkbox AS=/Off must override V=/Yes when AP/N named-state present');
}
if (asYes.type !== 'checkbox' || asYes.value !== 'Yes' || asYes.default_value !== 'Off') {
  throw new Error('checkbox AS=/Yes must override V=/Off when AP/N named-state present');
}
if (noAp.type !== 'checkbox' || noAp.value !== 'Yes' || noAp.default_value !== null) {
  throw new Error('checkbox without AP must keep V and null default');
}
...
const report = {
  ...
  providerProof: {
    checkboxAsOverridesVOff: true,
    checkboxAsOverridesVYes: true,
    checkboxAsNoApKeepsV: true,
  },
...
console.error(
  `v3.0.14 form checkbox-as-value residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`
);
