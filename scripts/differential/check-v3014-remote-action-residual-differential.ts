...
import { canonicalRemoteActionResidualResult, type Json } from './v3014-remote-action-residual-projection.ts';
import { resolveCargoReleaseDir } from '../native/cargo-target-dir.ts';
const scriptDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(scriptDir, '../..');
const fixtureDir = join(repoRoot, 'test/fixtures/differential');
const corpusPath = join(scriptDir, 'fixtures/v3014-remote-action-residual-corpus.json');
const oraclePath = join(scriptDir, 'fixtures/v3014-remote-action-residual-oracle.json');
const runnerPath = join(scriptDir, 'v3014-remote-action-residual-baseline-runner.ts');
const projectionPath = join(scriptDir, 'v3014-remote-action-residual-projection.ts');
...
if (commit !== baseline.commit || commit !== '92651c79c6ce8d10dfa3c76332176c26f222bd78') throw new Error('v3.0.14 remote action residual tag moved');
if (git('rev-parse', `${commit}^{tree}`).toString().trim() !== baseline.tree) throw new Error('v3.0.14 remote action residual tree mismatch');
for (const [name, path, expected] of [['runner', runnerPath, baseline.runnerSha256], ['projection', projectionPath, baseline.projectionSha256], ['corpus', corpusPath, baseline.corpusSha256],   ['launchStringFixture', launchStringFixture, baseline.fixtureSha256['v3014-annotation-launch-file-v1.pdf']!],
  ['launchDictFixture', launchDictFixture, baseline.fixtureSha256['v3014-annotation-launch-filedict-v1.pdf']!],
  ['gotorFixture', gotorFixture, baseline.fixtureSha256['v3014-annotation-gotor-v1.pdf']!]] as const) {
  if (sha256(readFileSync(path)) !== expected) throw new Error(`remote action residual ${name} digest drift`);
}
for (const [path, expected] of Object.entries(baseline.entrypointSha256)) {
  if (sha256(git('show', `${commit}:${path}`)) !== expected) throw new Error(`remote action residual entrypoint digest drift: ${path}`);
}
if (sha256(git('show', `${commit}:bun.lock`)) !== baseline.bunLockSha256) throw new Error('remote action residual bun.lock digest drift');
...
const invoke = async (entry: Case, root: string): Promise<Json> => {
  ...
  const request = (id: number, method: string, params: unknown): Promise<Record<string, unknown>> => new Promise((resolve, reject) => {
    const timer = setTimeout(() => { reject(new Error(`Rust remote action residual timed out: ${stderr.slice(-2000)}`)); }, 120_000);
    pending.set(id, (value) => { clearTimeout(timer); resolve(value); });
    child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id, method, params })}\n`);
  });
  try {
    await request(1, 'initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'v3014-remote-action-residual-rust', version: '1' } });
    ...
    return canonicalRemoteActionResidualResult(await request(2, 'tools/call', { name: 'read_pdf', arguments: input }));
...
const relocatedDir = mkdtempSync(join(tmpdir(), 'pdf-reader-remote-action-relocated-'));
try {
  copyFileSync(launchStringFixture, join(relocatedDir, 'v3014-annotation-launch-file-v1.pdf'));
  copyFileSync(launchDictFixture, join(relocatedDir, 'v3014-annotation-launch-filedict-v1.pdf'));
...
for (const [id, expectation] of Object.entries(oracle.expectations)) {
  for (const path of leaves(expectation)) {
    if (same(observations[id]!, mutate(expectation, path))) throw new Error(`remote action residual comparator missed ${id}.${path.join('.')}`);
    leafMutationCount += 1;
  }
}
...
if (ann('launch-string-file').url !== 'evil.exe' || ann('launch-string-file').has_dest !== false) throw new Error('launch string residual mismatch');
if (ann('launch-file-dict-uf').url !== 'report-u.pdf' || ann('launch-file-dict-uf').has_dest !== false) throw new Error('launch UF residual mismatch');
if (ann('gotor-remote-dest').url !== 'other.pdf#[0,{"name":"Fit"}]' || ann('gotor-remote-dest').has_dest !== false) throw new Error('gotor residual mismatch');
...
const report = {
  ...
  providerProof: { launchStringFile: true, launchFileDictPrefersUf: true, gotorRemoteDestJson: true },
...
console.error(`v3.0.14 remote action residual differential: PASS (${String(corpus.cases.length)}/${String(corpus.cases.length)}, zero skipped)`);
