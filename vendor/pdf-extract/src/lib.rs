// Vendored pdf-extract 0.12.1 (MIT) with the anymd Form XObject patch.
#![allow(warnings)]
extern crate lopdf;
...
impl<'a> Processor<'a> {
    ...
    fn process_stream(&mut self, doc: &'a Document, content: Vec<u8>, resources: &'a Dictionary, media_box: &MediaBox, output: &mut dyn OutputDev, page_num: u32) -> Result<(), OutputError> {
        self.process_stream_with_ctm(doc, content, resources, media_box, output, page_num, Transform2D::identity(), 0)
    }

    // anymd patch: Form XObjects inherit the invoking CTM and apply their
        ...
        for operation in &content.operations {
            ...
            match operation.operator.as_ref() {
                ...
                "Do" => {
                    ...
                    let is_form = xf.dict.get(b"Subtype").ok().and_then(|s| s.as_name().ok()) == Some(&b"Form"[..]);
                    if is_form && depth < 12 {
                        let resources = maybe_get_obj(&doc, &xf.dict, b"Resources").and_then(|n| n.as_dict().ok()).unwrap_or(resources);
                        let matrix = xf.dict.get(b"Matrix").ok().and_then(|m| m.as_array().ok()).filter(|m| m.len() == 6).map(|m| {
                            Transform2D::row_major(as_num(&m[0]), as_num(&m[1]), as_num(&m[2]), as_num(&m[3]), as_num(&m[4]), as_num(&m[5]))
